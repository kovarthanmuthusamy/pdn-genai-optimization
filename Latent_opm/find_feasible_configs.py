"""Find feasible configurations from a trained VAE surrogate.

Goal:
    Find decoded (occupancy, heatmap) configurations that satisfy:
        impedance(f) < target_impedance(f)   for all frequencies f

This is a *sampling + filtering* script (no gradient optimization). It is useful to:
- estimate feasibility rate for each K,
- quickly extract a few feasible solutions,
- sanity-check whether the trained surrogate can represent feasible regions.

Run:
    python Latent_opm/find_feasible_configs.py

Outputs:
    Latent_opm/runs/feasible_samples_<checkpoint>_<timestamp>/Kxx/sample_000/...
"""
