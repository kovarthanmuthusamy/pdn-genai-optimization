...
`New_heatmaps/all_combinations.csv` still listed all 49,106 decap combinations, so ECADStar batch `.peb` files were simulating deleted layouts.
...
```bash
# Preview
python New_heatmaps/filter_combinations_from_manifest.py
...
python New_heatmaps/change_frequency.py
...
```bash
python Data_Creation/Data_processing_multifreq.py --append --freqs-mhz 230 300 450 550 600
