...
(\texttt{docs/ecadstar\_headless\_cli.md}; \texttt{active\_\allowbreak{}learning\_\allowbreak{}pi/\allowbreak{}al/\allowbreak{}ecadstar.py}). One
...
The launcher (\texttt{pipelines/\allowbreak{}dataset\_\allowbreak{}sim/\allowbreak{}ecadstar.py}, function
...
\texttt{scrap/\allowbreak{}generation/\allowbreak{}generate\_\allowbreak{}peb\_\allowbreak{}multitype.py} covers only the multi-type pipeline} \\
\hline
\end{tabular}
\end{table}
...
(\texttt{pipelines/\allowbreak{}dataset\_\allowbreak{}sim/\allowbreak{}run\_\allowbreak{}combinations\_\allowbreak{}sim\_\allowbreak{}pipeline.py}).}
...
Input CSV & \texttt{data/\allowbreak{}heatmaps/\allowbreak{}peb\_\allowbreak{}with\_\allowbreak{}29k/\allowbreak{}all\_\allowbreak{}combinations\_\allowbreak{}merged.csv} \\
...
\texttt{datasets/\allowbreak{}data\_\allowbreak{}multifreq\_\allowbreak{}train/\allowbreak{}dataset\_\allowbreak{}meta.json}; the append queue in
...
(\texttt{active\_\allowbreak{}learning\_\allowbreak{}pi/\allowbreak{}al/\allowbreak{}peb\_\allowbreak{}batch.py}, \texttt{build\_peb\_from\_selection}), carrying a
per-sample frequency formatted as \texttt{"\{int(round(mhz))\}e6"}. The builder sets
\texttt{include\_distribution=True} and \texttt{include\_\allowbreak{}spectrum = not heatmap\_\allowbreak{}only}. In the
...
(\texttt{active\_\allowbreak{}learning\_\allowbreak{}pi/\allowbreak{}config/\allowbreak{}exp059\_\allowbreak{}gp\_\allowbreak{}error.json}, key \texttt{ecadstar}).}
...
Label ingestion (\texttt{active\_\allowbreak{}learning\_\allowbreak{}pi/\allowbreak{}al/\allowbreak{}ingest\_\allowbreak{}labels.py}) reads \texttt{PI-1} through
...
Grid size & \texttt{64} \\
Stored tensor & \texttt{np.stack([Zi * mask\_\allowbreak{}board, mask\_\allowbreak{}board], axis=0)}: channel 0 is the masked
...
\texttt{datasets/\allowbreak{}data\_\allowbreak{}multifreq\_\allowbreak{}train/\allowbreak{}manifest.csv} and
\texttt{datasets/\allowbreak{}data\_\allowbreak{}multifreq\_\allowbreak{}train\_\allowbreak{}norm\_\allowbreak{}unbounded/\allowbreak{}normalization\_\allowbreak{}stats.json} are matched by
...
Path & Role \\
\hline
\texttt{datasets/data\_multifreq\_train} & raw training set \\
\texttt{datasets/\allowbreak{}data\_\allowbreak{}multifreq\_\allowbreak{}train\_\allowbreak{}norm\_\allowbreak{}unbounded} & normalized set;
training input for exp059 \\
\texttt{datasets/\allowbreak{}data\_\allowbreak{}multifreq\_\allowbreak{}train\_\allowbreak{}norm\_\allowbreak{}robust} & bounded robust
normalization, earlier experiments \\
\texttt{datasets/\allowbreak{}data\_\allowbreak{}multifreq\_\allowbreak{}train\_\allowbreak{}K5} & $K\le5$ subset \\
\texttt{datasets/\allowbreak{}data\_\allowbreak{}multifreq\_\allowbreak{}al\_\allowbreak{}overlay\_\allowbreak{}exp057} & AL overlay
(legacy) \\
\texttt{datasets/\allowbreak{}data\_\allowbreak{}multifreq\_\allowbreak{}al\_\allowbreak{}overlay\_\allowbreak{}exp058} & AL overlay
(legacy) \\
\texttt{datasets/\allowbreak{}data\_\allowbreak{}multifreq\_\allowbreak{}al\_\allowbreak{}overlay\_\allowbreak{}exp059} & AL overlay
(current track) \\
\texttt{data\_multi\_norm\_robust} & older backup at the repository root \\
\hline
\end{tabular}
\end{table}

\texttt{datasets/\allowbreak{}data\_\allowbreak{}multifreq\_\allowbreak{}al\_\allowbreak{}overlay\_\allowbreak{}exp060} is referenced by
\texttt{experiments/\allowbreak{}exp060\_\allowbreak{}multitype\_\allowbreak{}occ/\allowbreak{}config.yaml} line 23 but does not exist on disk.

The current model track, \texttt{exp059\_capacity\_freq}, trains from \texttt{data\_dir}
\texttt{datasets/\allowbreak{}data\_\allowbreak{}multifreq\_\allowbreak{}train\_\allowbreak{}norm\_\allowbreak{}unbounded}. The primary AL configuration uses the
same \texttt{data\_dir}, \texttt{normalization\_stats\_json}
\texttt{datasets/\allowbreak{}data\_\allowbreak{}multifreq\_\allowbreak{}train\_\allowbreak{}norm\_\allowbreak{}unbounded/\allowbreak{}normalization\_\allowbreak{}stats.json} and
\texttt{overlay\_data\_dir} \texttt{datasets/\allowbreak{}data\_\allowbreak{}multifreq\_\allowbreak{}al\_\allowbreak{}overlay\_\allowbreak{}exp059}.
...
(\texttt{data/\allowbreak{}heatmaps/\allowbreak{}merged\_\allowbreak{}combinations\_\allowbreak{}report.json}, generated
...
is \texttt{data/\allowbreak{}heatmaps/\allowbreak{}peb\_\allowbreak{}with\_\allowbreak{}29k/\allowbreak{}all\_\allowbreak{}combinations\_\allowbreak{}merged.csv}, which has 29499 lines.
The output path recorded in the merge report itself
(\texttt{data/\allowbreak{}heatmaps/\allowbreak{}all\_\allowbreak{}combinations\_\allowbreak{}merged.csv}) no longer exists at that location; see
...
\texttt{pipelines/\allowbreak{}data/\allowbreak{}append\_\allowbreak{}merged\_\allowbreak{}combinations\_\allowbreak{}multifreq.py} \\
\texttt{generated\_at\_utc} & \texttt{2026-08-05T19:28:42.489387+00:00} \\
\texttt{dataset\_root} & \texttt{\allowbreak datasets/data\_multifreq\_train} \\
\texttt{storage} & \texttt{layout\_store} \\
...
\texttt{datasets/\allowbreak{}data\_\allowbreak{}multifreq\_\allowbreak{}train/\allowbreak{}dataset\_\allowbreak{}meta.json} lists 24 anchors, in MHz:
...
\texttt{freq\_mhz}, then \texttt{datasets/\allowbreak{}data\_\allowbreak{}multifreq\_\allowbreak{}train/\allowbreak{}dataset\_\allowbreak{}meta.json}, then the
legacy YAML defaults. That documentation statement was not verified against the implementation in
\texttt{src\_\allowbreak{}vae/\allowbreak{}others/\allowbreak{}multifreq\_\allowbreak{}anchors.py} line by line.
...
list in \texttt{datasets/\allowbreak{}data\_\allowbreak{}multifreq\_\allowbreak{}train/\allowbreak{}dataset\_\allowbreak{}meta.json}; decide which is stale and
...
\begin{table}[htbp]
\centering
\caption{\texttt{datasets/\allowbreak{}data\_\allowbreak{}multifreq\_\allowbreak{}train\_\allowbreak{}norm\_\allowbreak{}unbounded} (normalized; the exp059 training
input).}
\label{tab:appA_norm}
\begin{tabular}{p{0.40\textwidth} p{0.50\textwidth}}
\hline
...
\texttt{source\_dataset} & \texttt{\allowbreak datasets/data\_multifreq\_train} \\
\texttt{heatmap\_norm} & \texttt{robust\_\allowbreak{}log1p\_\allowbreak{}per\_\allowbreak{}mhz\_\allowbreak{}unbounded} \\
...
The budget filter is applied by \texttt{pipelines/\allowbreak{}normalize/\allowbreak{}build\_\allowbreak{}train\_\allowbreak{}norm\_\allowbreak{}unbounded.py},
which sets \texttt{DATA\_DIR} to \texttt{datasets/data\_multifreq\_train}, \texttt{OUTPUT\_DIR} to
\texttt{datasets/\allowbreak{}data\_\allowbreak{}multifreq\_\allowbreak{}train\_\allowbreak{}norm\_\allowbreak{}unbounded} and \texttt{MAX\_K} to 30, so only layouts
...
\begin{table}[htbp]
\centering
\caption{\texttt{datasets/\allowbreak{}data\_\allowbreak{}multifreq\_\allowbreak{}train\_\allowbreak{}K5} and its dry-run report.}
\label{tab:appA_k5}
\begin{tabular}{p{0.40\textwidth} p{0.50\textwidth}}
\hline
Field & Value \\
\hline
\texttt{source\_script} & \texttt{pipelines/\allowbreak{}data/\allowbreak{}copy\_\allowbreak{}multifreq\_\allowbreak{}k\_\allowbreak{}subset.py} \\
...
(\texttt{datasets/\allowbreak{}data\_\allowbreak{}multifreq\_\allowbreak{}train\_\allowbreak{}K5\_\allowbreak{}dry\_\allowbreak{}run\_\allowbreak{}report.json},
...
\texttt{datasets/\allowbreak{}data\_\allowbreak{}multifreq\_\allowbreak{}train\_\allowbreak{}K5\_\allowbreak{}dry\_\allowbreak{}run\_\allowbreak{}report.json} if the budget distribution is
...
\begin{table}[htbp]
\centering
\caption{\texttt{datasets/\allowbreak{}data\_\allowbreak{}multifreq\_\allowbreak{}al\_\allowbreak{}overlay\_\allowbreak{}exp059}, read from disk on 2026-08-05.}
...
\texttt{al\_\allowbreak{}heatmap\_\allowbreak{}only\_\allowbreak{}design\_\allowbreak{}ids.json} \\
\texttt{dataset\_meta.json} & absent \\
\texttt{design\_id} form &
e.g.\ \texttt{al\_\allowbreak{}al\_\allowbreak{}exp059\_\allowbreak{}gp\_\allowbreak{}error\_\allowbreak{}001\_\allowbreak{}i0001\_\allowbreak{}c009} \\
...
\texttt{datasets/\allowbreak{}data\_\allowbreak{}multifreq\_\allowbreak{}train\_\allowbreak{}norm\_\allowbreak{}unbounded/\allowbreak{}normalization\_\allowbreak{}stats.json}, key
\texttt{Heatmap}.}
...
Key & Value \\
\hline
\texttt{norm\_mode} & \texttt{robust\_\allowbreak{}log1p\_\allowbreak{}per\_\allowbreak{}mhz\_\allowbreak{}unbounded} \\
...
from \texttt{datasets/\allowbreak{}data\_\allowbreak{}multifreq\_\allowbreak{}train\_\allowbreak{}norm\_\allowbreak{}unbounded/\allowbreak{}normalization\_\allowbreak{}stats.json}}
...
\texttt{multifreq\_\allowbreak{}manifest.unique\_\allowbreak{}design\_\allowbreak{}id} & 24979 \\
\texttt{multifreq\_\allowbreak{}manifest.max\_\allowbreak{}k\_\allowbreak{}filter} & 30 \\
...
(\texttt{scrap/\allowbreak{}generation/\allowbreak{}generate\_\allowbreak{}peb\_\allowbreak{}multitype.py}, \texttt{TYPE\_CATALOG}).}
...
The combination list \texttt{data/\allowbreak{}heatmaps/\allowbreak{}combinations\_\allowbreak{}multitype\_\allowbreak{}combined\_\allowbreak{}15000.csv} holds
15000 rows of 52 integer type codes (0 empty, 1 type-1, 2 type-2). Its sampler report
(\texttt{data/\allowbreak{}heatmaps/\allowbreak{}combinations\_\allowbreak{}multitype\_\allowbreak{}sample\_\allowbreak{}report.json}, seed 42, generated
...
\texttt{pipelines/\allowbreak{}dataset\_\allowbreak{}sim/\allowbreak{}run\_\allowbreak{}multitype\_\allowbreak{}sim\_\allowbreak{}pipeline.py} reads the 15000-row CSV from row 0
...
(\texttt{pipelines/\allowbreak{}dataset\_\allowbreak{}sim/\allowbreak{}run\_\allowbreak{}append\_\allowbreak{}worker.py}) runs
\texttt{pipelines/\allowbreak{}data/\allowbreak{}append\_\allowbreak{}merged\_\allowbreak{}combinations\_\allowbreak{}multifreq.py} with \texttt{OUTPUT\_ROOT} set to
...
\texttt{datasets/\allowbreak{}data\_\allowbreak{}multifreq\_\allowbreak{}train/\allowbreak{}dataset\_\allowbreak{}meta.json} carries \texttt{generated\_at\_utc}
...
\texttt{datasets/\allowbreak{}data\_\allowbreak{}multifreq\_\allowbreak{}train/\allowbreak{}manifest.csv}, would.}
...
(\texttt{python -u pipelines/\allowbreak{}dataset\_\allowbreak{}sim/\allowbreak{}run\_\allowbreak{}multitype\_\allowbreak{}sim\_\allowbreak{}pipeline.py}) is no longer running,
...
\texttt{experiments/\allowbreak{}exp060\_\allowbreak{}multitype\_\allowbreak{}occ/\allowbreak{}config.yaml} sets \texttt{n\_decap\_types} to 2 (line
193) while still pointing \texttt{data\_dir} at
\texttt{datasets/\allowbreak{}data\_\allowbreak{}multifreq\_\allowbreak{}train\_\allowbreak{}norm\_\allowbreak{}unbounded} (line 134), which is the single-type
...
anchors &
\texttt{datasets/\allowbreak{}data\_\allowbreak{}multifreq\_\allowbreak{}train/\allowbreak{}dataset\_\allowbreak{}meta.json}, 24 anchors (the list actually loaded,
...
\texttt{datasets/\allowbreak{}data\_\allowbreak{}multifreq\_\allowbreak{}train\_\allowbreak{}norm\_\allowbreak{}unbounded/\allowbreak{}normalization\_\allowbreak{}stats.json} \\
Artifact staleness &
...
\texttt{data/\allowbreak{}heatmaps/\allowbreak{}combinations\_\allowbreak{}multitype\_\allowbreak{}sample\_\allowbreak{}report.json} and the CSV on disk: per-$K$
...
\texttt{data/\allowbreak{}heatmaps/\allowbreak{}all\_\allowbreak{}combinations\_\allowbreak{}merged.csv}, which is not present there &
the 29499-row merged CSV on disk is
\texttt{data/\allowbreak{}heatmaps/\allowbreak{}peb\_\allowbreak{}with\_\allowbreak{}29k/\allowbreak{}all\_\allowbreak{}combinations\_\allowbreak{}merged.csv}, which is what
\texttt{run\_\allowbreak{}combinations\_\allowbreak{}sim\_\allowbreak{}pipeline.py}:50 reads \\
Append worker root &
\texttt{run\_multitype\_sim\_pipeline.py}:120 sets
\texttt{TRIGGER\_APPEND\_AFTER\_MOVE=True} for the multi-type campaign &
\texttt{run\_append\_worker.py}:44--46 has \texttt{OUTPUT\_\allowbreak{}ROOT =
REPO\_\allowbreak{}ROOT/\allowbreak{}datasets/\allowbreak{}data\_\allowbreak{}multifreq\_\allowbreak{}train}, the single-type dataset, as its only output root \\
\hline
\end{tabular}
\end{table}
