...
Technical lineage of the multi-input product-of-experts (PoE) VAE used as a differentiable PDN surrogate. Losses are summarized here only where they changed with architecture; see [normalization-and-losses.md](./normalization-and-losses.md) for the full loss rationale.
...
See [training-procedure.md](./training-procedure.md) for the epoch loop.
...
4. PCB-slot GNN for occupancy (exp056) — rationale + numbers: [gnn-rationale.md](./gnn-rationale.md).
5. Structured 65-d latent + impedance spectrum GNN + occ↔imp coupling (exp057) — same doc for evidence vs over-claiming.

For normalization and loss literature, use [normalization-and-losses.md](./normalization-and-losses.md). For the AL fine-tune of exp057, use [active-learning.md](./active-learning.md).

**Later tracks (not a drop-in continuation of this doc’s architecture table):** exp058 (asymmetric KL / AL-path mix) and exp059 (higher capacity, multi-scale FiLM, spectral loss, layout holdout) — see [experiment-lineage.md](./experiment-lineage.md).
