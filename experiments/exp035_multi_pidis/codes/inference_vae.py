"""
Inference script for Multi-Input VAE — exp035_multi_pidis.
"""
import sys, os, json
import torch
import numpy as np
...
# CONFIGURATION
# ============================================================
exp = "experiments/exp035_multi_pidis"
...
PI_FREQ_MHZ  = 100        # PI frequency in MHz (e.g. 10, 100, 200, 500)
PI_FREQ      = (np.log10(PI_FREQ_MHZ * 1e6) - 5.0) / 4.0  # → log10-norm [0,1]
...
# ============================================================
class VAEInference:
    """Inference engine for generating samples from the exp035_multi_pidis trained VAE decoder."""
    ...
    def generate_save(self, num_samples=1, out_dir="temp_visuals", K=26, PI_freq=0.828):
        """Generate and save num_samples samples from the trained decoder."""
        with torch.no_grad():
            hm_z, occ, imp_norm = self.model.inference(
                num_samples, self.device, K=K, PI_freq=PI_freq,
...
def main():
    ...
    engine.generate_save(out_dir=OUTPUT_DIR, num_samples=NUM_SAMPLES, K=K_VALUE, PI_freq=PI_FREQ)
    print("\n✓ Inference complete!")
