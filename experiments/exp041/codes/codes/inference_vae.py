...
def _default_data_dir() -> Path:
    """Dataset dir from exp038 config.yaml, else multifreq_norm default."""
...
# CONFIGURATION
# ============================================================
exp = "experiments/exp038_true_multi"
...
PI_FREQ_MHZ  = 100        # PI frequency in MHz (converted inside model.inference)
...
# ============================================================
class VAEInference:
    """Inference engine for generating samples from the exp038 trained VAE decoder."""
    ...
    def __init__(self, checkpoint_path: str,
        ...
        # Normalization stats (project-root paths; matches training data_dir)
        ...
        self.background_value = ns.get("background_value", ns.get("Heatmap", {}).get("background_value", -2.8751))
        ...
        self.binary_mask      = np.load(cfg_dir / "binary_mask.npy")  # (64, 64) bool
    ...
    def generate_save(self, num_samples=1, out_dir="temp_visuals", K=26, PI_freq_mhz=200.0):
        ...
        with torch.no_grad():
            hm_z, occ, imp_norm = self.model.inference(
                num_samples, self.device, K=K, PI_freq=PI_freq_mhz, pi_freq_unit="mhz",
...
def main():
    ...
    engine.generate_save(
        out_dir=OUTPUT_DIR, num_samples=NUM_SAMPLES, K=K_VALUE, PI_freq_mhz=PI_FREQ_MHZ,
    )
    print("\n✓ Inference complete!")
