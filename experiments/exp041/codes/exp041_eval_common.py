"""Shared paths, model loading, and PI_freq helpers for exp041 evaluation scripts."""
...
def resolve_paths(cfg: dict[str, Any] | None = None) -> dict[str, Path | str]:
    ...
    data_dir = Path(cfg.get("data_dir", PROJECT_ROOT / "datasets/data_multifreq_norm"))
...
def pi_norm_to_mhz(pi_norm: np.ndarray | float) -> np.ndarray | float:
    """Invert log10-normalised PI_freq tensor → MHz."""
...
def load_model(
    ...
    device: torch.device,
) -> tuple[MultiInputVAE, dict[str, Any], dict | None, dict | None]:
