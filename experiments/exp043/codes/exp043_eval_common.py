...
def resolve_paths(cfg: dict[str, Any] | None = None) -> dict[str, Path | str]:
    ...
    data_dir = Path(cfg.get("data_dir", PROJECT_ROOT / "datasets/data_multifreq_gmax"))
...
def heatmap_to_physical(hm_norm: torch.Tensor, norm_stats: dict[str, Any]) -> torch.Tensor:
    """Denorm heatmap batch to Ω (z-score or global-max)."""
    from src_vae.others.heatmap_z_clip import heatmap_norm_to_physical

    hm = norm_stats["Heatmap"]
    lo, hi = hm.get("clip_min"), hm.get("clip_max")
    clip_lo = float(lo) if lo is not None else None
    clip_hi = float(hi) if hi is not None else None
    return heatmap_norm_to_physical(hm_norm, hm, clip_lo=clip_lo, clip_hi=clip_hi)
