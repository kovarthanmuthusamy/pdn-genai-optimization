...
class VAEInference:
    """Inference engine for exp043 multifreq PI heatmap VAE."""
    ...
    def __init__(
        ...
        self.hm_stats = hm
        self.norm_stats = ns
        self.hm_log_mean = float(hm.get("log_mean", 0.0))
        self.hm_log_std = float(hm.get("log_std", 1.0))
        self.use_global_max = hm.get("norm_mode") == "global_max" or hm.get("global_max_ohm") is not None
        self.global_max_ohm = float(hm["global_max_ohm"]) if self.use_global_max else None
        self.hm_z_clip = (
            (float(hm["clip_min"]), float(hm["clip_max"]))
        ...
        if hm.get("norm_mode") == "global_max" or hm.get("global_max_ohm") is not None:
            print(f"Heatmap (global-max): gmax={float(hm['global_max_ohm']):.4f} Ω")
        else:
    ...
    def generate_save(self, num_samples=1, out_dir="temp_visuals", K=26, PI_freq_mhz=200.0):
        with torch.no_grad():
            ...
            hm_phys = self.denorm_heatmap_physical(hm_z)
        ...
        def _process_one(i):
            ...
            norm_name = "heatmap_norm.npy" if self.use_global_max else "heatmap_zscore.npy"
            for fname, arr in {
                norm_name: hm_z_np[i],
