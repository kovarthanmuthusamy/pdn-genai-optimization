"""Training — Multi-Input VAE (exp043): global-max heatmaps + phased layout sweep."""
...
from experiments.exp043.codes.gmax_training_patch import (
    apply_gmax_config,
    patch_trainer,
    transform_disk_heatmap,
)
from experiments.exp043.codes.impedance_spectrum_loss import (
...
class Config:
    ...
    epoch_log_interval: int = 1  # print progress every N epochs (always prints ep 1, checkpoints, final)
    ...
    # Phased layout training: early epochs use posterior z more often (heatmap encoder recon grad).
    layout_train_prob: float = 0.95
    layout_train_prob_early: float = 0.4
    layout_train_prob_late: float = 0.95
    layout_train_phase_epoch: int = 150
    # Cross-freq loss always uses layout z (matches sweep / layout_cross eval).
    cross_freq_layout_z_only: bool = False
    # When set, layout encode for cross-freq uses this MHz (sweep pi_ref), decode at PI_freq_alt.
    cross_freq_layout_pi_ref_mhz: float | None = None
    # Push layout-z decode toward posterior-z decode on the same batch (same PI_freq).
    layout_distill_weight: float = 0.0
    # v5: GT heatmap encoder skips into layout/cross-freq decode (train only).
    layout_teacher_skips: bool = True
    # v5: layout encode at cross_freq_layout_pi_ref_mhz (sweep π_ref), decode at native MHz.
    layout_encode_pi_ref: bool = True
    # v5: distill layout@π_ref → decode@alt toward posterior@alt (matches sweep cross-freq).
    layout_distill_cross_freq: bool = True
    ...
    heatmap_pattern_weight: float = 0.0
    heatmap_spread_weight: float = 0.0
    heatmap_hotspot_weight: float = 0.0
    heatmap_peak_centroid_weight: float = 0.0
    # Blend [0,1] of quantile-inverse FG huber (upweights rare high train-space bins vs crowded low tail).
    heatmap_quantile_weight: float = 0.0
    heatmap_quantile_bins: int = 10
    heatmap_quantile_power: float = 0.75
    ...
    # exp043: global-max heatmap + synthetic MHz blend + cross-freq full loss
    synthetic_blend_prob: float = 0.12
    use_freq_poe_expert: bool = True
    use_global_max_heatmap: bool = False
    global_max_ohm: float | None = None  # set from normalization_stats.json via apply_gmax_config
    heatmap_train_space: str = "linear"  # "linear" | "log1p_gmax"
    heatmap_fg_threshold: float | None = None  # None → dataset fg_norm_threshold
    heatmap_phys_p99_over_weight: float = 0.5
    heatmap_huber_delta: float = 0.2
    heatmap_grad_huber_delta: float = 0.1
    heatmap_dynrange_over_weight: float = 0.5
    cross_freq_full_loss: bool = True
...
LOSS_KEYS = (
    ...
    "impedance_legacy_loss", "impedance_peak_loss", "cross_freq_heatmap_loss", "layout_distill_loss",
    "heatmap_phys_p99_loss",
...
def _iter_expert_stats(expert_stats: dict | None):
    """Yield ``(name, mu, logvar)`` — skips ``heatmap_skips`` and other metadata."""
    if not expert_stats:
        return
    for name, val in expert_stats.items():
        if name == "heatmap_skips":
            continue
        if not isinstance(val, tuple) or len(val) != 2:
            continue
        m, lv = val
        if torch.is_tensor(m) and torch.is_tensor(lv):
            yield name, m, lv


def _expert_mu_lv_pairs(expert_stats: dict | None) -> list[tuple[torch.Tensor, torch.Tensor]]:
    """(mu, logvar) tuples only — skips ``heatmap_skips`` and other metadata."""
    return [(m, lv) for _, m, lv in _iter_expert_stats(expert_stats)]
...
def vae_loss(recon_hm, recon_occ, recon_imp, tgt_hm, tgt_occ, tgt_imp,
    ...
    experts = _expert_mu_lv_pairs(expert_stats)
    if experts and c.per_expert_kl_weight > 0:
        exp_kl = sum(
            (-0.5 * (1.0 + lv - m.pow(2) - lv.exp())).mean() for m, lv in experts
        ) / len(experts)
        total = total + c.per_expert_kl_weight * exp_kl
    if c.sigma_reg_weight > 0 and experts:
        prec = torch.stack([1.0 / (lv.exp() + 1e-8) for _, lv in experts]).sum(0) + 1.0
...
def _layout_pi_ref_norm(pi: torch.Tensor, c: Config) -> torch.Tensor:
    """Sweep-aligned π_ref for layout encode (default 200 MHz from config)."""
    pi_ref_mhz = getattr(c, "cross_freq_layout_pi_ref_mhz", None)
    if pi_ref_mhz is None:
        return pi
    from src_vae.others.pi_freq_utils import pi_freq_mhz_to_norm

    return torch.full_like(pi, pi_freq_mhz_to_norm(float(pi_ref_mhz)))


def _teacher_skips(expert_stats: dict, c: Config) -> dict[str, torch.Tensor] | None:
    if not getattr(c, "layout_teacher_skips", True):
        return None
    return expert_stats.get("heatmap_skips")
...
def _layout_train_prob(epoch: int, c: Config) -> float:
    """Early training favors posterior z (heatmap encoder recon); later matches layout sweep."""
    phase = int(getattr(c, "layout_train_phase_epoch", 0))
    if phase <= 0:
        return float(c.layout_train_prob)
    if epoch < phase:
        return float(getattr(c, "layout_train_prob_early", 0.4))
    return float(getattr(c, "layout_train_prob_late", c.layout_train_prob))
...
def _forward_train_batch(
    ...
    *,
    ...
    torch.Tensor, torch.Tensor, dict, torch.Tensor, torch.Tensor,
]:
    ...
    skips = _teacher_skips(expert_stats, c)
    layout_p = _layout_train_prob(epoch, c)
    use_layout = (
        train
        and layout_p > 0.0
        and bool((torch.rand((), device=pi.device) < layout_p).item())
    )
    if use_layout:
        pi_lay = _layout_pi_ref_norm(pi, c) if getattr(c, "layout_encode_pi_ref", True) else pi
        z = base.encode_layout_latent(occ, imp, K, pi_lay)
    else:
        z = z_post
    pi_dec = _jitter_pi_freq_norm(pi, c, train=train)
    rh, ro, ri = base.decode(
        z,
        K,
        pi_dec,
        occupancy=occ,
        heatmap_skips=skips if use_layout else expert_stats.get("heatmap_skips"),
    )
    return rh, ro, ri, mu, logvar, expert_stats, z, z_post
...
def _cross_modal(model, hm_enc, occ, imp, hm, K, pi, c: Config, imp_std: float, ps: float) -> torch.Tensor:
    ...
    for src in ("heatmap", "impedance"):
        ...
        rh, ro, ri = model.decode(z, K, pi, occupancy=occ)
...
def _clean_previous_run(exp: Path) -> None:
    """Deprecated: exp043 no longer deletes old runs (kept for backward compatibility)."""
    print(f"Fresh run — preserving existing outputs under {exp}")
    print("  (new run will be written under experiments/exp043/runs/...)")


def _new_run_dir(exp: Path) -> Path:
    """Create a unique run directory under exp/runs/ (never deletes existing data)."""
    from datetime import datetime, timezone

    runs = exp / "runs"
    runs.mkdir(parents=True, exist_ok=True)
    ts = datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ")
    # Ensure uniqueness if two starts happen same second.
    for i in range(1000):
        name = f"run_{ts}" if i == 0 else f"run_{ts}_{i:03d}"
        p = runs / name
        try:
            ...
            continue
    raise RuntimeError("Could not allocate unique run directory under runs/")
...
def print_curriculum_summary(c: Config) -> None:
    print(
        ...
        f"layout_train={c.layout_train_prob_early}→{c.layout_train_prob_late} "
        f"@epoch{c.layout_train_phase_epoch}, "
        f"cross_freq_layout_z_only={c.cross_freq_layout_z_only}, "
        f"cross_freq_pi_ref={getattr(c, 'cross_freq_layout_pi_ref_mhz', None)}, "
        f"layout_distill_w={getattr(c, 'layout_distill_weight', 0.0)}, "
        f"layout_teacher_skips={getattr(c, 'layout_teacher_skips', False)}, "
        f"layout_encode_pi_ref={getattr(c, 'layout_encode_pi_ref', False)}, "
        f"layout_distill_cf={getattr(c, 'layout_distill_cross_freq', False)}",
    )
    ...
    *, train: bool, optimizer=None, scaler=None,
    ...
    with ctx:
        for i, batch in enumerate(loader):
            ...
            with autocast:
                if train:
                    rh, ro, ri, mu, lv, ex, z, z_post = _forward_train_batch(
                        model, hm_enc, occ, imp, K, pi, c, train=True, epoch=epoch,
                    )
                else:
                ...
                if (
                    ...
                    hm_alt_lin = batch["heatmap_norm_alt"].to(c.device, non_blocking=c.is_cuda())
                    hm_alt = transform_disk_heatmap(hm_alt_lin, c)
                    base_cf = getattr(model, "_orig_mod", model)
                    t_skips = _teacher_skips(ex, c)
                    if c.cross_freq_layout_z_only:
                        z_cf = base_cf.encode_layout_latent(occ, imp, K, _layout_pi_ref_norm(pi, c))
                    else:
                        z_cf = z
                    cf = _cross_freq_heatmap_loss(
                        ...
                        occupancy=occ,
                        heatmap_skips=t_skips,
                    )
                    losses["cross_freq_heatmap_loss"] = cf
                    losses["total_loss"] = losses["total_loss"] + cf_w * cf
                ld_w = float(getattr(c, "layout_distill_weight", 0.0))
                if train and ld_w > 0:
                    from experiments.exp043.codes.gmax_heatmap_loss import heatmap_loss_gmax

                    t_skips = _teacher_skips(ex, c)
                    z_lay = base.encode_layout_latent(occ, imp, K, _layout_pi_ref_norm(pi, c))
                    if (
                        getattr(c, "layout_distill_cross_freq", True)
                        and "heatmap_norm_alt" in batch
                    ):
                        pi_tgt = batch["PI_freq_alt"].to(c.device, non_blocking=c.is_cuda())
                        rh_teacher, _, _ = base.decode(
                            z_post, K, pi_tgt, occupancy=occ, heatmap_skips=t_skips,
                        )
                        rh_lay, _, _ = base.decode(
                            z_lay, K, pi_tgt, occupancy=occ, heatmap_skips=t_skips,
                        )
                    else:
                        pi_dec = _jitter_pi_freq_norm(pi, c, train=True)
                        rh_teacher, _, _ = base.decode(
                            z_post, K, pi_dec, occupancy=occ, heatmap_skips=t_skips,
                        )
                        rh_lay, _, _ = base.decode(
                            z_lay, K, pi_dec, occupancy=occ, heatmap_skips=t_skips,
                        )
                    ld = heatmap_loss_gmax(
                        rh_lay, rh_teacher.detach(), c, ps, lite=True,
                    ).mean()
                    losses["layout_distill_loss"] = ld
                    losses["total_loss"] = losses["total_loss"] + ld_w * ld
            ...
            if train:
                ...
                sig_sum += sc.sum(0)
                n_mu += mc.shape[0]
                for name, em, elv in _iter_expert_stats(ex):
...
def _should_print_epoch(ep: int, c: Config, *, checkpoint: bool) -> bool:
    interval = max(1, int(getattr(c, "epoch_log_interval", 1)))
    if checkpoint or ep == 1 or ep == c.num_epochs:
        return True
...
def _on_stats_loaded(c: Config, raw: dict) -> None:
    """Apply global-max heatmap patches when dataset uses gmax normalization."""
    if apply_gmax_config(c, raw):
        patch_trainer(sys.modules[__name__], c)
    elif getattr(c, "use_global_max_heatmap", False):
        raise ValueError(
            "use_global_max_heatmap is enabled but normalization_stats.json "
            "does not contain global-max Heatmap stats — rebuild data_multifreq_gmax first.",
        )


def build_vae_model(c: Config) -> MultiInputVAEPoeFreq:
    """Build exp043 PoE-freq model with optional heatmap-private latent tail."""
    priv = int(getattr(c, "heatmap_private_dim", 0))
    model = MultiInputVAEPoeFreq(
        latent_dim=c.latent_dim,
        cond_dim=c.cond_dim,
        heatmap_private_dim=priv,
    ...
    model._hm_out_scale = float(c.heatmap_z_clip_max) if getattr(c, "heatmap_z_clip_max", None) else 1.02
    latent_mode = (
        f"unified latent_dim={c.latent_dim}"
        if priv == 0
        else f"shared={model.shared_latent_dim}+private={priv}"
    )
    print(
        f"  exp043 model: residual heatmap bottleneck + freq PoE expert "
        f"({latent_mode}, poe={getattr(c, 'use_freq_poe_expert', True)})",
    )
    return model
...
def train_vae() -> None:
    ...
    # Fresh start: write outputs into a new run directory (never delete old runs).
    if not c.resume_checkpoint:
        _clean_previous_run(exp)
        run_exp = _new_run_dir(exp)
        print(f"Fresh run — outputs → {run_exp}")
        exp = run_exp
        c.experiment_dir = str(exp)
    ...
    for epoch in range(start, c.num_epochs):
        ...
        if _should_print_epoch(ep, c, checkpoint=log_metrics):
            if log_metrics:
                print(
        ...
        if save_ckpt:
            ...
            # Do not delete checkpoints automatically; preserve full run history.
            print(f"  checkpoint → {ckpt_path.name}")
            if c.eval_off_anchor_mhz:
                from experiments.exp043.codes.eval_cross_freq_gmax import run_off_anchor_eval_gmax

                run_off_anchor_eval_gmax(
                    model,
                    val_ld,
                    c=c,
...
if __name__ == "__main__":
    ...
    if cfg_path.is_file():
        data_dir = load_experiment_config(cfg_path).get("data_dir")
        if data_dir:
