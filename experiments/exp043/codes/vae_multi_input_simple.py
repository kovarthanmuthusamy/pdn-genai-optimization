...
HM_OUT_SCALE = 1.02  # sigmoid-bounded decoder output in norm space [0, HM_OUT_SCALE]
...
class MultiInputVAE(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        self.heatmap_dec_skip32 = OptionalSkipFuse(32, 32, 32)
        self.heatmap_dec_occ_fuse = (
            nn.Sequential(
                nn.Conv2d(33, 32, kernel_size=3, padding=1),
                nn.GroupNorm(8, 32),
                nn.LeakyReLU(),
            )
            if use_occ_spatial_decoder
            else None
        )
    ...
    def _decode_heatmap_tensor(
        ...
        hm_feat = self.heatmap_dec_skip32(hm_feat, skip16)

        if self.use_occ_spatial_decoder and self.heatmap_dec_occ_fuse is not None and occupancy is not None:
        ...
        logits = self.heatmap_dec_out(hm_feat)
        scale = getattr(self, "_hm_out_scale", HM_OUT_SCALE)
    ...
    def forward(self, heatmap, occupancy, impedance, K, PI_freq):
        ...
        hm_recon, occ_recon, imp_recon = self.decode(
            ...
            PI_freq,
            occupancy=occupancy,
            heatmap_skips=expert_stats.get("heatmap_skips"),
        )
