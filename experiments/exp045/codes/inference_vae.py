"""
Inference script for Multi-Input VAE — exp045.
...
def _default_data_dir() -> Path:
    ...
    return PROJECT_ROOT / "datasets" / "data_multifreq_norm_z_score"
...
from experiments.exp045.codes.vae_poe_freq import MultiInputVAEPoeFreq  # noqa: E402
...
class VAEInference:
    """Inference engine for exp045 multifreq PI heatmap VAE."""
    ...
    def denorm_heatmap_physical(self, hm_norm: torch.Tensor) -> torch.Tensor:
        """Denorm model heatmap output to physical Ω (log1p z-score)."""
        ...
        return heatmap_z_to_physical(
            hm_norm, self.hm_log_mean, self.hm_log_std, clip_lo=lo, clip_hi=hi,
        )
    ...
    def generate_save(self, num_samples=1, out_dir="temp_visuals", K=26, PI_freq_mhz=200.0):
        with torch.no_grad():
            ...
            hm_phys = heatmap_z_to_physical(
                hm_z, self.hm_log_mean, self.hm_log_std, clip_lo=lo, clip_hi=hi,
            )
