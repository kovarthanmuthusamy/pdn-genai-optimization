...
### The split latent
```
latent_dim 48 = shared_latent_dim 28 + heatmap_private_dim 20
```
...
1. **`layout_private_head`** (new module in the model) — MLP `[occ_feat(64) + imp_feat(64) + cond(K+freq)] → heatmap_private_dim`. Added as an extra PoE expert in `encode_layout_latent_full`, playing the role the heatmap expert plays during encode. The layout latent now fills all 48 dims.
...
- **Increased capacity** — latent_dim 48, heatmap_private_dim 20
