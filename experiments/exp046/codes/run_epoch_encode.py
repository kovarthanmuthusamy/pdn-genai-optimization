...
def _latent_distill_loss(mu_student: torch.Tensor, mu_teacher: torch.Tensor, c: Config, base) -> torch.Tensor:
    ...
    heatmap provides during encode.
    """
    tgt = mu_teacher.detach()
    shared = base.shared_latent_dim
    priv = base.heatmap_private_dim
    if priv > 0:
        d_shared = (mu_student[:, :shared] - tgt[:, :shared]).pow(2).mean()
        d_priv = (mu_student[:, shared:] - tgt[:, shared:]).pow(2).mean()
        pm = float(getattr(c, "latent_distill_private_mult", 3.0))
        return d_shared + pm * d_priv
    return (mu_student - tgt).pow(2).mean()
...
def _forward_train_batch(
    ...
    *,
    train: bool,
) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, dict, torch.Tensor, torch.Tensor]:
    """Encode for KL; decode with teacher U-Net skips on encode batches.

    Also computes latent distillation: layout (student) latent → encode (teacher).
    """
    ...
    if train and w_distill > 0.0 and use_head:
        z_lay, mu_lay, _ = base.encode_layout_latent_full(occ, imp, K, pi)
        distill = _latent_distill_loss(mu_lay, mu, c, base)
    ...
    return rh, ro, ri, mu, logvar, expert_stats, z, distill
    ...
    with ctx:
        for i, batch in enumerate(loader):
            ...
            with autocast:
                distill = None
                if train:
                    rh, ro, ri, mu, lv, ex, z, distill = _forward_train_batch(
