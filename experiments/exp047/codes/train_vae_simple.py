...
from experiments.exp047.codes.run_epoch_encode import _run_epoch as _run_epoch_encode
from experiments.exp047.codes.spatial_metrics import peak_loc_loss, pearson_fg_loss
from experiments.exp047.codes.synthetic_freq_blend import maybe_apply_synthetic_blend
from experiments.exp047.codes.vae_poe_freq import MultiInputVAEPoeFreq
...
def build_vae_model(c: Config) -> MultiInputVAEPoeFreq:
    ...
    c._model_ref = model
    print(
        f"  exp047 model: private_h={c.layout_private_hidden}  freq_film={c.use_layout_private_freq_film}  "
...
def _patch_dataloader() -> None:
    from experiments.exp047.codes import dataloader_multifreq as _dm047
...
def _patch_spatial_eval() -> None:
    ...
    from experiments.exp047.codes.eval_spatial_metrics import run_off_anchor_eval_spatial

    if getattr(_eval_cf, "_exp047_spatial_patched", False):
        return
    ...
    _eval_cf._exp047_spatial_patched = True


def _print_training_summary(c: _tr.Config) -> None:
    enc_pct = (1.0 - float(c.layout_train_prob)) * 100.0
    print(
        "  exp047 training (high-freq layout focus, no early stop):\n"
...
def main() -> None:
    ...
    if cfg_path.is_file():
        ...
        os.environ.setdefault("VAE_DATA_DIR", str(_PROJECT_ROOT / "data_multi_norm"))
    train_vae()
