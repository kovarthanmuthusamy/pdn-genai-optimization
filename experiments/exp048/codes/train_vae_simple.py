...
from experiments.exp048.codes.run_epoch_encode import _run_epoch as _run_epoch_encode
from experiments.exp048.codes.spatial_metrics import peak_loc_loss, pearson_fg_loss
from experiments.exp048.codes.synthetic_freq_blend import maybe_apply_synthetic_blend
from experiments.exp048.codes.vae_poe_freq import MultiInputVAEPoeFreq
...
class Config(_tr.Config):
    ...
    layout_sharpening_weight: float = 1.0
...
def build_vae_model(c: Config) -> MultiInputVAEPoeFreq:
    ...
    c._model_ref = model
    print(
        f"  exp048 model: private_h={c.layout_private_hidden}  freq_film={c.use_layout_private_freq_film}  "
        f"layout_prob={c.layout_train_prob}  cf_layout_mix={c.cross_freq_layout_mix_prob}\n"
        f"    distill_w={c.latent_distill_weight}  out_distill_w={c.output_distill_weight}  "
        f"layout_sharpen_w={c.layout_sharpening_weight}  freeze_occ_imp_epoch={c.freeze_occ_imp_epoch}",
    )
    return model
...
def _on_stats_loaded(c: Config, raw: dict) -> None:
    """Attach centralized norm bundle; widen clip bounds for robust per-MHz stats."""
...
def _patch_dataloader() -> None:
    from experiments.exp048.codes import dataloader_multifreq as _dm047
...
def _patch_spatial_eval() -> None:
    ...
    from experiments.exp048.codes.eval_spatial_metrics import run_off_anchor_eval_spatial

    if getattr(_eval_cf, "_exp048_spatial_patched", False):
        return
    ...
    _eval_cf._exp048_spatial_patched = True


def _print_training_summary(c: _tr.Config) -> None:
    enc_pct = (1.0 - float(c.layout_train_prob)) * 100.0
    print(
        "  exp048 training (high-freq layout focus, no early stop):\n"
        ...
        f"peak_w={c.heatmap_peak_weight}  grad_w={c.heatmap_grad_weight}\n"
        f"    layout_sharpen_w={c.layout_sharpening_weight}  hf_mult={c.layout_high_freq_loss_mult}  "
        f"private_h={c.layout_private_hidden}  freq_film={c.use_layout_private_freq_film}\n"
...
def main() -> None:
    ...
    if cfg_path.is_file():
        ...
        os.environ.setdefault("VAE_DATA_DIR", str(_PROJECT_ROOT / "data_multi_norm_robust"))
    train_vae()
