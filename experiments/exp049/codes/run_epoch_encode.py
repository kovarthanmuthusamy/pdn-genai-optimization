...
def _heatmap_phys_amplitude_loss(
    recon: torch.Tensor,
    target: torch.Tensor,
    c: Config,
    *,
    ...
    """Foreground p99 in physical Ω — uses centralized norm_stats when available."""
    ...
    apply_clip = True
    if bundle is not None and bundle.heatmap.is_unbounded():
        apply_clip = False
    ...
    p99_r = _tr._percentile_along_dim(flat_r, q, dim=1, max_samples=1024)
    p99_t = _tr._percentile_along_dim(flat_t, q, dim=1, max_samples=1024)
    under = F.relu(p99_t - p99_r).pow(2)
    over_w = float(getattr(c, "heatmap_phys_p99_over_weight", 0.0))
    over = F.relu(p99_r - p99_t).pow(2)
        ...
        for i, batch in enumerate(loader):
            ...
            with autocast:
                ...
                if c.heatmap_phys_p99_weight > 0:
                    phys = _heatmap_phys_amplitude_loss(rh, hm, c, pi_freq=pi)
