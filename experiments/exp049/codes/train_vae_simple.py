...
from experiments.exp049.codes.run_epoch_encode import _run_epoch as _run_epoch_encode
from experiments.exp049.codes.spatial_metrics import peak_loc_loss, pearson_fg_loss
from experiments.exp049.codes.synthetic_freq_blend import maybe_apply_synthetic_blend
from experiments.exp049.codes.vae_poe_freq import MultiInputVAEPoeFreq
...
class Config(_tr.Config):
    ...
    heatmap_phys_p99_over_weight: float = 0.75
...
def build_vae_model(c: Config) -> MultiInputVAEPoeFreq:
    ...
    c._model_ref = model
    print(
        f"  exp049 model: private_h={c.layout_private_hidden}  freq_film={c.use_layout_private_freq_film}  "
...
def _patch_dataloader() -> None:
    from experiments.exp049.codes import dataloader_multifreq as _dm047
...
def _patch_spatial_eval() -> None:
    ...
    if getattr(_eval_cf, "_exp049_spatial_patched", False):
        return
    ...
    _eval_cf._exp049_spatial_patched = True


def _print_training_summary(c: _tr.Config) -> None:
    enc_pct = (1.0 - float(c.layout_train_prob)) * 100.0
    print(
        "  exp049 training (high-freq layout focus, no early stop):\n"
