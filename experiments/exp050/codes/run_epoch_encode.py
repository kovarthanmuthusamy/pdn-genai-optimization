"""Training epoch loop for exp050 — pixel peak blob losses + layout path."""
...
from experiments.exp050.codes.heatmap_peak_losses import heatmap_peak_blob_phys_loss
    ...
    with ctx:
        for i, batch in enumerate(loader):
            ...
            with autocast:
                ...
                phys_w = _peak_phys_loss_weight(
                    pi, c, float(getattr(c, "heatmap_peak_phys_weight", 0.0)),
                )
                if phys_w > 0.0:
                    phys = heatmap_peak_blob_phys_loss(rh, hm, c, pi_freq=pi)
                    losses["heatmap_peak_phys_loss"] = phys
                    losses["total_loss"] = losses["total_loss"] + phys_w * phys
