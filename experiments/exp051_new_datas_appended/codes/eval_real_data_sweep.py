...
Run:
  ...
  python -m experiments.exp051_new_datas_appended.codes.eval_real_data_sweep
"""
...
from experiments.exp051_new_datas_appended.codes.exp051_eval_common import (
    load_exp_config,
    load_model,
    pi_norm_to_mhz,
    resolve_paths,
)
from experiments.exp051_new_datas_appended.codes.spatial_metrics import peak_loc_err, pearson_fg
from experiments.exp038_true_multi.codes.dataloader_multifreq import (
    create_multifreq_data_loaders,
)
from src_vae.others.norm_stats import HeatmapNormStats, load_norm_stats
    ...
    *,
    ...
    for bi, batch in enumerate(val_loader):
        ...
        for target_mhz in eval_mhz:
            ...
            hm_real_phys = hm_stats.norm_to_physical(
                hm_sel, pi_norm=pi_sel, apply_clip=not hm_stats.is_unbounded(),
            )
            ...
            for kind, recon in (("encode", rh_enc), ("layout", rh_lay)):
                ...
                recon_phys = hm_stats.norm_to_physical(
                    recon, pi_norm=pi_sel, apply_clip=not hm_stats.is_unbounded(),
                )
...
def main() -> None:
    ...
    bundle = load_norm_stats(paths["data_dir"], stats_path=paths["norm_stats"])
    hm_stats = bundle.heatmap
    ...
    print(f"  denorm: {hm_stats.describe()}")
    print()
