"""Shared paths, model loading, and PI_freq helpers for exp051 evaluation scripts."""
...
from experiments.exp051_new_datas_appended.codes.vae_poe_freq import MultiInputVAEPoeFreq  # noqa: E402
...
def resolve_paths(cfg: dict[str, Any] | None = None) -> dict[str, Path | str]:
    ...
    data_dir = resolve_repo_path(
        cfg.get("data_dir", "datasets/data_multifreq_train_norm_robust"),
    )
    ...
    if not ckpt.is_file():
        ...
        if ep is not None:
            if isinstance(ep, int):
                alt = EXP_DIR / "checkpoints" / f"checkpoint_epoch_{ep}.pt"
            elif isinstance(ep, str) and ("/" in ep or ep.endswith(".pt")):
                alt = resolve_repo_path(ep)
            else:
