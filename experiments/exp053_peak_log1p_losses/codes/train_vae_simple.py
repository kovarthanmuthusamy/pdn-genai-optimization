...
from experiments.exp053_peak_log1p_losses.codes.run_epoch_encode import _run_epoch as _run_epoch_encode
from experiments.exp053_peak_log1p_losses.codes.heatmap_peak_losses import (
    heatmap_loss_pearson_grad,
    heatmap_peak_log1p_total,
)
from experiments.exp053_peak_log1p_losses.codes.spatial_metrics import peak_loc_sharp_loss
from experiments.exp053_peak_log1p_losses.codes.synthetic_freq_blend import maybe_apply_synthetic_blend
from experiments.exp053_peak_log1p_losses.codes.vae_poe_freq import MultiInputVAEPoeFreq
...
class Config(_tr.Config):
    ...
    heatmap_peak_loc_weight: float = 0.8
    heatmap_peak_loc_temperature: float = 0.03
    heatmap_pearson_weight: float = 2.5
    # exp053 log1p peak stack (hotspot / max / p95 / centroid / local-max)
    heatmap_peak_hotspot_weight: float = 2.5
    heatmap_peak_max_log1p_weight: float = 1.75
    heatmap_peak_p95_log1p_weight: float = 0.5
    heatmap_peak_centroid_weight: float = 2.5
    heatmap_peak_centroid_top_q: float = 0.95
    heatmap_peak_local_max_weight: float = 1.0
    heatmap_peak_log1p_weight: float = 3.5
    ...
    # exp053: no legacy peak/dynrange/p99 heatmap terms or recon z-clip
    ...
    heatmap_peak_topk_k: int = 32
    heatmap_peak_phys_weight: float = 1.0
    heatmap_peak_phys_overshoot_weight: float = 2.5
    heatmap_peak_phys_undershoot_weight: float = 3.5
    heatmap_peak_phys_hf_mult: float = 2.5
    ...
    heatmap_mhz_loss_weight_max: float = 3.0
    # NaN guards
...
def _print_hm_breakdown(epoch_1based: int) -> None:
    from experiments.exp053_peak_log1p_losses.codes.run_epoch_encode import _LAST_TRAIN_METRICS

    if epoch_1based <= 1 or not _LAST_TRAIN_METRICS:
        return
    tier = _LAST_TRAIN_METRICS.get("heatmap_loss_tier_a")
    pk = _LAST_TRAIN_METRICS.get("heatmap_peak_log1p_loss")
    hm = _LAST_TRAIN_METRICS.get("heatmap_loss")
    phys = _LAST_TRAIN_METRICS.get("heatmap_peak_phys_loss")
    if tier is None or pk is None:
        return
    print(
        f"  [hm ep {epoch_1based - 1}] total={hm:.4f}  tier_a={tier:.4f}  "
        f"peak_log1p={pk:.4f}  phys_blob={phys:.4f}",
        flush=True,
    )
...
def _on_train_epoch_start(epoch_1based: int, c: Config, train_ld) -> None:
    ...
    from experiments.exp053_peak_log1p_losses.codes.sampler_curriculum import apply_layout_path_curriculum

    _print_hm_breakdown(epoch_1based)
    apply_layout_path_curriculum(c, epoch_1based)
    sampler = getattr(train_ld, "exp053_curriculum_sampler", None)
...
def build_vae_model(c: Config) -> MultiInputVAEPoeFreq:
    ...
    c._model_ref = model
    print(
        f"  exp053 model: private_h={c.layout_private_hidden}  freq_film={c.use_layout_private_freq_film}  "
        ...
        f"peak_log1p=hot{c.heatmap_peak_hotspot_weight}/max{c.heatmap_peak_max_log1p_weight}/ctr{c.heatmap_peak_centroid_weight}  "
...
def _heatmap_loss_with_spatial(
    ...
    **_,
    ...
    if peak_loc_w > 0 and not lite:
        ...
        fg = (target > bg).float()
        temp = float(getattr(c, "heatmap_peak_loc_temperature", 0.05))
        loss = loss + peak_loc_w * peak_loc_sharp_loss(recon, target, fg, bg, temperature=temp)
    return loss
...
def _on_stats_loaded(c: Config, raw: dict) -> None:
    """Attach unbounded norm bundle (exp053 requires unbounded stats)."""
    ...
    if not hm.is_unbounded():
        raise ValueError(
            f"exp053 requires unbounded heatmap stats; got {hm.norm_mode!r} (unbounded={hm.unbounded})",
        )
...
def _vae_loss_mhz_weighted(
    ...
    peak_w = float(getattr(c, "heatmap_peak_log1p_weight", 0.0))
    if peak_w > 0.0:
        peak_per, _terms = heatmap_peak_log1p_total(
            recon_hm, tgt_hm, c, pi_freq=pi_freq,
        )
        if sample_w is not None:
            peak_loss = weighted_mean(peak_per.reshape(-1), sample_w)
        else:
            peak_loss = peak_per.reshape(-1).mean()
        losses["heatmap_loss_tier_a"] = losses["heatmap_loss"].detach()
        losses["heatmap_peak_log1p_loss"] = peak_loss
        losses["total_loss"] = losses["total_loss"] + peak_w * peak_loss
        # Include peak terms in heatmap_loss metric for epoch logs (not recon_loss).
        losses["heatmap_loss"] = losses["heatmap_loss"] + peak_w * peak_loss

    return sanitize_loss_dict(losses, c)


def _append_heatmap_peak_split_csv(metrics_dir: Path, epoch: int, tr: dict, val: dict | None) -> None:
    ...
    with path.open("a", newline="", encoding="utf-8") as f:
        ...
        if write_header:
            w.writerow([
                "epoch",
                "train_heatmap_loss", "train_tier_a", "train_peak_log1p", "train_phys_blob",
                "val_heatmap_loss", "val_tier_a", "val_peak_log1p", "val_phys_blob",
            ])
        w.writerow([
            epoch,
            tr.get("heatmap_loss", ""),
            tr.get("heatmap_loss_tier_a", ""),
            tr.get("heatmap_peak_log1p_loss", ""),
            tr.get("heatmap_peak_phys_loss", ""),
            val.get("heatmap_loss", "") if val else "",
            val.get("heatmap_loss_tier_a", "") if val else "",
            val.get("heatmap_peak_log1p_loss", "") if val else "",
            val.get("heatmap_peak_phys_loss", "") if val else "",
        ])
...
def _patch_training() -> None:
    ...
    _tr.LOSS_KEYS = _tr.LOSS_KEYS + ("heatmap_peak_phys_loss", "heatmap_peak_log1p_loss", "heatmap_loss_tier_a")
    _patch_spatial_eval()
    _patch_dataloader()
    ...
    _orig_append_impedance_split = _tr._append_impedance_split_csv

    def _append_exp053_split_csvs(metrics_dir, epoch, tr, val):
        _orig_append_impedance_split(metrics_dir, epoch, tr, val)
        _append_heatmap_peak_split_csv(metrics_dir, epoch, tr, val)

    _tr._append_impedance_split_csv = _append_exp053_split_csvs
    _tr._apply_yaml_config = _apply_yaml_with_epoch_hook


def _patch_dataloader() -> None:
    from experiments.exp053_peak_log1p_losses.codes import dataloader_multifreq as _dm047
...
def _patch_spatial_eval() -> None:
    ...
    from experiments.exp053_peak_log1p_losses.codes.eval_spatial_metrics import run_off_anchor_eval_spatial

    if getattr(_eval_cf, "_exp053_spatial_patched", False):
        return
    ...
    _eval_cf._exp053_spatial_patched = True


def _print_training_summary(c: _tr.Config) -> None:
    enc_pct = (1.0 - float(c.layout_train_prob)) * 100.0
    print(
        "  exp053 training (Pearson + grad + log1p peak stack + union blob):\n"
        ...
        f"    pearson_w={c.heatmap_pearson_weight}  grad_vec={c.heatmap_grad_vector_weight}  "
        f"grad_dir={c.heatmap_grad_direction_weight}\n"
        f"    peak_log1p_w={getattr(c, 'heatmap_peak_log1p_weight', 0)}  "
        f"hot={getattr(c, 'heatmap_peak_hotspot_weight', 0)}  "
        f"max={getattr(c, 'heatmap_peak_max_log1p_weight', 0)}  "
        f"ctr={getattr(c, 'heatmap_peak_centroid_weight', 0)}\n"
        f"    peak_topk={getattr(c, 'heatmap_peak_topk_k', 32)}  "
        f"phys_w={getattr(c, 'heatmap_peak_phys_weight', 0)}  "
        f"peak_loc_w={c.heatmap_peak_loc_weight}\n"
        f"    epochs={c.num_epochs}  amp={getattr(c, 'amp', 'off')}  "
        f"resume={getattr(c, 'resume_checkpoint', None)}",
    )
