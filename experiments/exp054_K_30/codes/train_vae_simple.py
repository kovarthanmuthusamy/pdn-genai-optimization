...
import experiments.exp054_K_30.codes.train_core as _tr
from experiments.exp054_K_30.codes.exp054_common import EXP_DIR, load_yaml_config, vae_model_kwargs
from experiments.exp054_K_30.codes.run_epoch_encode import _run_epoch as _run_epoch_encode
from experiments.exp054_K_30.codes.heatmap_peak_losses import ExtremaSide, heatmap_extrema_log1p_both, heatmap_loss_pearson_grad
from experiments.exp054_K_30.codes.spatial_metrics import extrema_loc_sharp_loss
from experiments.exp054_K_30.codes.vae_poe_freq import MultiInputVAEPoeFreq
from experiments.exp054_K_30.codes.distributed_train import is_main_process
...
class Config(_tr.Config):
    # exp054 additions (not present in exp038 base Config)
    ...
    # exp054: training loop / stability knobs
...
def build_vae_model(c: Config) -> MultiInputVAEPoeFreq:
    ...
    if is_main_process():
        print(f"Model: {n/1e6:.1f}M params  latent={c.latent_dim}  layout_p={c.layout_train_prob}  "
              f"cf_mix={c.cross_freq_layout_mix_prob}  milestones=[{sched}]", flush=True)
    return model
...
def _on_stats_loaded(c: Config, raw: dict) -> None:
    ...
    if not bundle.heatmap.is_unbounded():
        raise ValueError(f"exp054 requires unbounded heatmap stats; got {bundle.heatmap.norm_mode!r}")
...
def _vae_loss_exp054(recon_hm, recon_occ, recon_imp, tgt_hm, tgt_occ, tgt_imp, mu, logvar, beta, c,
                     expert_stats, *, epoch, K, physics, pw, pi_freq, imp_log_std, ps, apply_k, weight_overrides=None):
    from experiments.exp054_K_30.codes.training_guard import clamp_logvar, sanitize_loss_dict
...
def _apply_yaml_exp054(c: Config) -> None:
    p = Path(c.experiment_dir) / "config.yaml"
    if not p.is_file():
        raise FileNotFoundError(f"exp054 requires config.yaml at {p}")
    data = _tr.load_experiment_config(p)
    explicit = {k for k, v in data.items() if k.endswith("_epoch") and isinstance(v, (int, float)) and not isinstance(v, bool)}
    for key, val in data.items():
        if key.startswith("_"):
            continue
        setattr(c, key, tuple(float(x) for x in val) if key == "eval_off_anchor_mhz" and isinstance(val, list) else val)
    _tr.apply_curriculum_epochs(c, skip_epoch_keys=explicit)
    print(f"Loaded config from {p.name} ({len(data)} keys)")
    _tr.print_curriculum_summary(c)
    c.on_train_epoch_start = _on_train_epoch_start
    from experiments.exp054_K_30.codes.eval_off_anchor import set_off_anchor_config
...
def _impedance_weights_exp054(c: Config):
    from experiments.exp054_K_30.codes.impedance_spectrum_loss import ImpedanceSpectrumWeights
...
def _impedance_loss_exp054(recon, target, c, imp_log_std, ps, *, epoch):
    from experiments.exp054_K_30.codes.impedance_spectrum_loss import impedance_spectrum_loss
    return impedance_spectrum_loss(
        recon, target,
        imp_log_std=imp_log_std,
        penalty_scale=ps,
        peak_scale=_tr._peak_loss_scale(epoch, c),
        w=_impedance_weights_exp054(c),
    )



def _patch_training() -> None:
    from experiments.exp054_K_30.codes import dataloader_multifreq as dm
    _tr.Config, _tr.build_vae_model, _tr._run_epoch = Config, build_vae_model, _run_epoch_encode
    _tr.heatmap_loss, _tr.vae_loss, _tr._on_stats_loaded = _heatmap_loss_with_spatial, _vae_loss_exp054, _on_stats_loaded
    ...
    _tr._apply_yaml_config = _apply_yaml_exp054
    _tr.impedance_loss = _impedance_loss_exp054
...
def train_vae() -> None:
    ...
    if is_main_process():
        print(f"exp054  epochs={c.num_epochs}  batch={c.batch_size}  amp={c.amp}  ckpt_every={c.checkpoint_interval}  "
