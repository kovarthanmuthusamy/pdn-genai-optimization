...
import experiments.exp055_hard_occ.codes.train_core as _tr
from experiments.exp055_hard_occ.codes.exp055_common import EXP_DIR, load_yaml_config, vae_model_kwargs
from experiments.exp055_hard_occ.codes.run_epoch_encode import _run_epoch as _run_epoch_encode
from experiments.exp055_hard_occ.codes.heatmap_peak_losses import ExtremaSide, heatmap_extrema_log1p_both, heatmap_loss_pearson_grad
from experiments.exp055_hard_occ.codes.spatial_metrics import extrema_loc_sharp_loss
from experiments.exp055_hard_occ.codes.vae_poe_freq import MultiInputVAEPoeFreq
from experiments.exp055_hard_occ.codes.distributed_train import is_main_process
...
def _vae_loss_exp055(recon_hm, recon_occ, recon_imp, tgt_hm, tgt_occ, tgt_imp, mu, logvar, beta, c,
    ...
    from experiments.exp055_hard_occ.codes.training_guard import clamp_logvar, sanitize_loss_dict
...
def _apply_yaml_exp055(c: Config) -> None:
    ...
    from experiments.exp055_hard_occ.codes.eval_off_anchor import set_off_anchor_config
...
def _impedance_weights_exp055(c: Config):
    from experiments.exp055_hard_occ.codes.impedance_spectrum_loss import ImpedanceSpectrumWeights
...
def _impedance_loss_exp055(recon, target, c, imp_log_std, ps, *, epoch):
    from experiments.exp055_hard_occ.codes.impedance_spectrum_loss import impedance_spectrum_loss
...
def _patch_training() -> None:
    from experiments.exp055_hard_occ.codes import dataloader_multifreq as dm
...
def train_vae() -> None:
    ...
    if is_main_process():
        print(f"exp055  epochs={c.num_epochs}  batch={c.batch_size}  amp={c.amp}  ckpt_every={c.checkpoint_interval}  "
