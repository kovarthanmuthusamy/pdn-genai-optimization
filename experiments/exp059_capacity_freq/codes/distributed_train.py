...
def init_distributed() -> tuple[int, int, int]:
    ...
    if not dist.is_initialized():
        # Main-only val/off-anchor can leave the other rank waiting on broadcast/barrier;
        # default 10min NCCL timeout is too tight if rank0 is slow. Override via env.
        timeout_min = float(os.environ.get("NCCL_TIMEOUT_MINUTES", "30"))
        dist.init_process_group(
            backend="nccl",
            timeout=__import__("datetime").timedelta(minutes=timeout_min),
        )
