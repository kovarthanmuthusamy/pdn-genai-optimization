...
from experiments.exp059_capacity_freq.codes.graph_occ import OccGraphDecoder, OccGraphEncoder
from experiments.exp059_capacity_freq.codes.graph_imp import ImpSpectrumGraphDecoder, ImpSpectrumGraphEncoder
...
class MultiInputVAE(nn.Module):
    ...
    def decode(
        ...
        if occupancy is not None and getattr(self, "binary_occupancy_decode", False):
            from experiments.exp059_capacity_freq.codes.occupancy_binary import occupancy_for_heatmap_decode
            occupancy = occupancy_for_heatmap_decode(
                occupancy, K,
                force_binary=True,
                ste=getattr(self, "occupancy_binary_ste", False),
            )

        hm_recon = self._decode_heatmap_tensor(z, cond, occupancy=occupancy)

        occ_recon = self.occupancy_decoder(dec_occ)
        occ_for_imp = occupancy if occupancy is not None else torch.sigmoid(occ_recon)
