"""Multi-frequency PI heatmap dataloader (vendored for exp055).
...
# Multi-type one-hot width: set by create_* via set_n_decap_types (default single type).
N_DECAP_TYPES: int = 1


def set_n_decap_types(n: int) -> None:
    global N_DECAP_TYPES
    N_DECAP_TYPES = max(1, int(n))


def _occ_to_onehot_tensor(occ: torch.Tensor) -> torch.Tensor:
    from experiments.exp060_multitype_occ.codes.occupancy_types import type_ids_to_onehot
    if occ.dim() == 2 and occ.shape[0] == 52:
        # already (N, C)?
        from experiments.exp060_multitype_occ.codes.occupancy_types import n_occ_classes
        if occ.shape[1] == n_occ_classes(N_DECAP_TYPES):
            return occ.float()
    if occ.dim() == 1 or (occ.dim() == 2 and occ.shape[-1] == 52):
...
def _split_indices_by_design(
    ...
    """True layout holdout: each design_id (all its MHz rows) goes entirely to
    train OR val. Deterministic given ``seed`` + sorted design keys.

    Previous behaviour split frequencies *within* each design (same layout in
    both splits) — that leaked layout identity into val and is unsuitable for
    GP / AL residual evaluation. This version holds out whole layouts.
    """
    ...
    perm = torch.randperm(n_d, generator=rng).tolist()
    n_train_d = int(n_d * train_split)
    # Keep at least one design on each side when possible.
    if n_d >= 2:
        n_train_d = max(1, min(n_d - 1, n_train_d))
    else:
        n_train_d = n_d

    train_designs = {designs[j] for j in perm[:n_train_d]}
    train_indices: list[int] = []
    val_indices: list[int] = []
    for dk in designs:
        idxs = by_design[dk]
        if dk in train_designs:
...
def multifreq_collate_fn(batch: list) -> dict:
    out = {
        ...
        "occupancy": torch.stack([_occ_to_onehot_tensor(b["occupancy"]) for b in batch]),
    ...
    if batch and all("sample_idx" in b for b in batch):
        out["sample_idx"] = torch.tensor([b["sample_idx"] for b in batch], dtype=torch.long)
    if "heatmap_only" in batch[0]:
        out["heatmap_only"] = torch.stack([b["heatmap_only"] for b in batch])
    return out
...
def create_multifreq_data_loaders(
    ...
    *,
    ...
    set_n_decap_types(n_decap_types)
    ...
    if split_by_design:
        ...
        print(f"Multifreq LAYOUT holdout (seed={seed}): {len(set(design_keys))} designs -> train_rows={len(train_indices)} val_rows={len(val_indices)} (all MHz of a design stay together)")
    else:
        n = len(dataset)
