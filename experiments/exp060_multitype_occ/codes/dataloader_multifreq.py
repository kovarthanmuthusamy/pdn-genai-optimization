...
import experiments.exp060_multitype_occ.codes.dataloader_base as dataloader_base
from experiments.exp060_multitype_occ.codes.dataloader_base import (
    DataLoader,
    Optional,
    Path,
    RandomSampler,
...
def create_multifreq_data_loaders(
    ...
    *,
    ...
    dataloader_base.set_n_decap_types(n_decap_types)
    ...
    if split_by_design:
        ...
        n_train_layouts = len({design_keys[i] for i in train_idx})
        n_val_layouts = len({design_keys[i] for i in val_idx})
        print(
            f"Multifreq LAYOUT holdout (seed={seed}): "
            f"{n_layouts} designs -> train={n_train_layouts} val={n_val_layouts} "
            f"(all MHz of a design stay together; no layout overlap)",
            flush=True,
        )
    else:
        n = len(dataset)
