...
from experiments.exp060_multitype_occ.codes.spatial_metrics import peak_loc_err, pearson_fg
...
def run_off_anchor_eval_spatial(
    ...
    *,
    ...
    model.eval()
    from experiments.exp060_multitype_occ.codes.distributed_train import unwrap_model
    ...
    for bi, batch in enumerate(val_loader):
        ...
        from experiments.exp060_multitype_occ.codes.occupancy_binary import occupancy_for_heatmap_decode
        from experiments.exp060_multitype_occ.codes.occupancy_types import ensure_batch_onehot

        n_types = int(getattr(base, "n_occ_classes", 2))
        occ = ensure_batch_onehot(occ, max(1, n_types))
        occ_hm = occupancy_for_heatmap_decode(occ, K, force_binary=use_binary_occupancy) if use_binary_occupancy else occ
        if use_occ_only_layout:
            z_layout = base.encode_occupancy_latent(occ, K, pi_native, sample=False)
        else:
        ...
        for mhz in off_anchor_mhz:
            ...
            rh_enc, _, _ = base.decode(z_enc_native, K, pi_t, occupancy=occ)
