"""
Inference script for Multi-Input VAE — exp060 (no U-Net skips).
...
from experiments.exp060_multitype_occ.codes.exp060_common import vae_model_kwargs  # noqa: E402
from experiments.exp060_multitype_occ.codes.vae_poe_freq import MultiInputVAEPoeFreq  # noqa: E402
...
class VAEInference:
    """Inference engine for exp060 multifreq PI heatmap VAE (no U-Net skips)."""
    ...
    def __init__(
        ...
        yaml_cfg: dict = {}
        if _CONFIG_PATH.is_file():
            yaml_cfg = load_experiment_config(_CONFIG_PATH)

        # Merge yaml + checkpoint config; dims forced from checkpoint.
        arch = {**yaml_cfg, **{k: v for k, v in ckpt_cfg.items() if v is not None}}
        ld = int(ckpt_cfg.get("latent_dim", arch.get("latent_dim", latent_dim)))
        cond = int(ckpt_cfg.get("cond_dim", arch.get("cond_dim", 8)))
        hm_priv = int(ckpt_cfg.get("heatmap_private_dim", arch.get("heatmap_private_dim", 8)))
        mod_drop = float(ckpt_cfg.get("modality_dropout", arch.get("modality_dropout", 0.0)))
        arch["latent_dim"] = ld
        arch["cond_dim"] = cond
        arch["heatmap_private_dim"] = hm_priv
        arch["modality_dropout"] = mod_drop

        # Infer flags that change module structure from weight keys.
        # (freq_poe_mu always exists even when use_freq_poe_expert=False — use config for that.)
        keys = set(ckpt_state)
        arch["use_multiscale_film"] = any(k.startswith("heatmap_dec_film_bottleneck.") for k in keys)
        arch["use_heatmap_film"] = any(k.startswith("heatmap_dec_film.") for k in keys) or bool(
            arch.get("use_heatmap_film", True)
        )
        if "use_freq_poe_expert" not in arch:
            arch["use_freq_poe_expert"] = False
        arch.pop("use_heatmap_unet_skips", None)

        kw = vae_model_kwargs(arch)
        kw.pop("use_heatmap_unet_skips", None)
        print(
            f"Model: latent_dim={ld}  cond_dim={cond}  heatmap_private_dim={hm_priv}  "
            f"modality_dropout={mod_drop}  multiscale_film={kw.get('use_multiscale_film')}  "
            f"freq_poe={kw.get('use_freq_poe_expert')}"
        )
        self.model = MultiInputVAEPoeFreq(**kw)
        ...
        if bool(kw.get("use_freq_poe_expert", False)):
            required_prefixes = required_prefixes + ("freq_poe_mu.",)
        if bool(kw.get("use_layout_private_head", False)):
            required_prefixes = required_prefixes + ("layout_private_mu.",)
        if bool(kw.get("use_multiscale_film", False)):
            required_prefixes = required_prefixes + ("heatmap_dec_film_bottleneck.",)
        ...
        if skipped:
            raise RuntimeError(
                f"Checkpoint has {len(skipped)} tensors that do not match the exp060 model "
                f"(architecture mismatch). First: {skipped[:8]}"
            )
    ...
    def heatmap_train_to_disk(self, hm_train: torch.Tensor) -> torch.Tensor:
        """Model train-space heatmap → on-disk array for sweep/compare.

        Unbounded robust_log1p_per_mhz: train space is already the stored norm.
        """
        return hm_train
    ...
    def generate_save(self, num_samples=1, out_dir="temp_visuals", K=26, PI_freq_mhz=200.0):
        with torch.no_grad():
            ...
            from experiments.exp060_multitype_occ.codes.occupancy_types import (
                hard_onehot_from_logits,
                onehot_to_type_ids,
                presence_from_onehot,
            )
            if occ.dim() == 3:
                occ_bin = hard_onehot_from_logits(occ, K)  # (B,52,C)
                occ_type = onehot_to_type_ids(occ_bin)     # (B,52) for CAD-ish export
                occ_pres = presence_from_onehot(occ_bin)
            else:
        ...
        occ_prob_np = (occ.softmax(-1).cpu().numpy() if occ.dim() == 3 else occ.cpu().numpy())
        occ_np = occ_pres.cpu().numpy()
        ...
        def _process_one(i):
            ...
            for fname, arr in {
                ...
                "occupancy_map.npy": occ_pres.cpu().numpy()[i],
                "type_map.npy": occ_type.cpu().numpy()[i],
                "occupancy_onehot.npy": occ_bin.cpu().numpy()[i],
            ...
            self.visualize_sample(
                ...
                occupancy=occ_np[i],
                occupancy_prob=(occ_pres.cpu().numpy()[i] if occ.dim() == 3 else occ_prob_np[i]),
                sample_idx=i,
                output_dir=plots,
                K=K,
            )
