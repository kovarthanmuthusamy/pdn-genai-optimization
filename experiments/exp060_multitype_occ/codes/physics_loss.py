...
class PhysicsCritic(nn.Module):
    """
    Dual-branch CNN: decap occupancy grid + PI_freq → spatial effectiveness map.

    PI_freq at API boundaries is in MHz (converted internally); training batches
    pass log10-normalised scalars in [0, 1].
    The heatmap PI distribution is frequency-conditioned; the critic must see
    freq so RI supervision matches the sample's operating point.

    Strategy (see PhysicsLoss docstring):
      - Supervision: invert min-max heatmap FG → effectiveness target on real pairs.
      - RI constraint: penalise recon heatmap pixels that are "cold" (low z) where
        the critic says decap coverage is weak (detached critic during VAE step).
    """
    def __init__(self, freq_emb_dim: int = 16):
        super().__init__()
        self.occ_conv1 = nn.Conv2d(1, 16, kernel_size=3, padding=1)
        self.occ_conv2 = nn.Conv2d(16, 32, kernel_size=3, padding=1)
        ...
        self.fuse = nn.Sequential(
            nn.Conv2d(32 + freq_emb_dim, 32, kernel_size=3, padding=1),
    ...
    @staticmethod
    def _pi_freq_batch(
        pi_freq: torch.Tensor | float | None,
        batch_size: int,
        device: torch.device,
        dtype: torch.dtype,
        *,
        ...
        """(B, 1) PI_freq in [0, 1]. ``unit``: 'norm' (training) or 'mhz' (inference)."""
        t = pi_freq_norm_for_model(
            pi_freq, batch_size, unit=unit, device=device, dtype=dtype,
        )
        return t.view(batch_size, 1)

    def forward(
        self,
        occ_spatial: torch.Tensor,
        pi_freq: torch.Tensor | float | None = None,
        pi_freq_unit: str = "norm",
    ) -> torch.Tensor:
        """occ_spatial: (B, 1, 7, 8) → effectiveness (B, 1, 64, 64) in [0, 1]."""
        B = occ_spatial.shape[0]
        x = F.interpolate(occ_spatial, size=(64, 64), mode='bilinear', align_corners=False)
        x = F.leaky_relu(self.occ_conv1(x), 0.1)
        x = F.leaky_relu(self.occ_conv2(x), 0.1)
        pf = self._pi_freq_batch(pi_freq, B, x.device, x.dtype, unit=pi_freq_unit)
        emb = self.freq_mlp(pf).view(B, -1, 1, 1).expand(-1, -1, 64, 64)
        return torch.sigmoid(self.fuse(torch.cat([x, emb], dim=1)))
...
class PhysicsLoss(nn.Module):
    """
    Physics-informed constraint losses for the Multi-Modal VAE (exp035).

    Critic strategy
    ---------------
    1. critic_supervision (warmup): PhysicsCritic(occ, PI_freq) is trained to
       predict an effectiveness map derived from the *target* heatmap FG.  Target
       construction: invert per-sample min-max normalised FG so well-decoupled
       (low z) regions → 1, background → 0.  PI_freq is required because the same
       decap layout yields different spatial PI patterns at different frequencies.

    2. radius_influence (VAE stage): For recon heatmap, penalise FG pixels that are
       unusually cold (low z) where critic effectiveness is low — i.e. the VAE
       should not claim strong decoupling without nearby decaps.  Critic weights
       are detached here so VAE gradients do not corrupt the critic.

    3. anti_resonance: Analytical peak-without-prior-dip penalty on impedance ch0
       (K-only decode; independent of PI_freq).

    Staged weights are applied in train_vae_simple._physics_stage_weights.
    """
    ...
    def critic_supervision_loss(
        ...
        """Supervise PhysicsCritic on real (occ, hm, PI_freq) triples."""
        occ_spatial   = self._occ_to_spatial(target_occ.float())
        pred_eff      = self.critic(occ_spatial, pi_freq, pi_freq_unit=pi_freq_unit)
    ...
    def radius_influence_loss(
        ...
        """Penalize heatmap pixels with low z but low freq-conditioned decap coverage."""
        occ_spatial   = self._occ_to_spatial(occ_prob)
        effectiveness = self.critic(occ_spatial, pi_freq, pi_freq_unit=pi_freq_unit).detach()
    ...
    def forward(
        ...
        """Returns 3 scalar losses: (ri, cs, ar). pi_freq: (B,) norm or MHz per unit."""
        # Presence (B,52) already in [0,1] from multi-type path; legacy path uses logits.
        if recon_occ_logits.dim() == 2 and recon_occ_logits.min() >= 0 and recon_occ_logits.max() <= 1.0 + 1e-4:
            occ_prob = recon_occ_logits
        else:
            occ_prob = torch.sigmoid(recon_occ_logits)

        ri_loss = self.radius_influence_loss(
            recon_hm, occ_prob, pi_freq, pi_freq_unit=pi_freq_unit,
        )
        ar_loss = self.anti_resonance_loss(recon_imp)

        cs_loss = recon_hm.new_zeros(())
        if target_occ is not None and target_hm is not None:
            cs_loss = self.critic_supervision_loss(
                target_occ, target_hm, pi_freq, pi_freq_unit=pi_freq_unit,
            )
