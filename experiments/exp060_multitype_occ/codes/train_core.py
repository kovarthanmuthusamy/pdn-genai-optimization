...
from experiments.exp060_multitype_occ.codes.dataloader_multifreq import create_multifreq_data_loaders
from src_vae.others.multifreq_layout_store import manifest_path
from experiments.exp060_multitype_occ.codes.impedance_spectrum_loss import (
    ImpedanceSpectrumWeights,
    impedance_spectrum_loss,
)
from experiments.exp060_multitype_occ.codes.physics_loss import PhysicsLoss
from experiments.exp060_multitype_occ.codes.vae_poe_freq import MultiInputVAEPoeFreq as MultiInputVAE
from experiments.exp060_multitype_occ.codes.distributed_train import (
...
class Config:
    ...
    # Multi-type schema: T = n_decap_types channels; empty = all-zero row. occ (B, 52, T).
    n_decap_types: int = 2
    ...
    experiment_dir: str = field(
        default_factory=lambda: os.environ.get("VAE_EXPERIMENT_DIR", "experiments/exp060_multitype_occ"),
    )
...
def _masked_batch_mean(values: torch.Tensor, include_mask: torch.Tensor | None) -> torch.Tensor:
    """Mean over batch; ``include_mask`` is 1 for samples that contribute to the loss."""
    if include_mask is None:
        return values.mean()
    m = include_mask.to(values.dtype)
    if values.dim() > 1:
        m = m.view(-1, *([1] * (values.dim() - 1)))
    denom = m.sum().clamp(min=1e-6)
    if values.dim() == 1:
        return (values * m).sum() / denom
    per = (values * m).sum(dim=tuple(range(1, values.dim()))) / m.sum(dim=tuple(range(1, values.dim()))).clamp(min=1e-6)
    return per.sum() / m.view(-1).sum().clamp(min=1e-6)
...
def vae_loss(recon_hm, recon_occ, recon_imp, tgt_hm, tgt_occ, tgt_imp,
             ...
             heatmap_only_mask: torch.Tensor | None = None) -> dict[str, torch.Tensor]:
    ...
    include = None
    if heatmap_only_mask is not None:
        include = (1.0 - heatmap_only_mask.to(recon_hm.dtype).view(-1)).clamp(0.0, 1.0)
    hm_ps = heatmap_loss(recon_hm, tgt_hm, c, ps, dynrange_weight=hdr)
    # Occupancy: focal BCE on (B,52,T) type channels (empty = all-zero target row).
    pos_w = None
    if K is not None:
        # weight positive type bits; broadcast over type channels if needed
        w = ((52.0 - K.float()) / K.float().clamp(min=1)).clamp(0.5, 10.0).unsqueeze(1)
        if recon_occ.dim() == 3:
            pos_w = w.unsqueeze(-1).expand_as(recon_occ)
        else:
            pos_w = w.expand_as(recon_occ)
    if tgt_occ.dim() == 3:
        # mean over slots and types → per-sample
        bce = F.binary_cross_entropy_with_logits(recon_occ, tgt_occ.clamp(0, 1), reduction="none")
        if pos_w is not None:
            bce = bce * pos_w
        gamma = compute_focal_gamma(epoch, c)
        if gamma > 0:
            with torch.no_grad():
                pt = torch.exp(-bce)
            bce = (1.0 - pt).pow(gamma) * bce
        occ_ps = bce.mean(dim=(1, 2))
    else:
    ...
    if apply_k and c.use_k_weighting and K is not None:
        ...
        loss_hm = _wmean(hm_ps, wh)
        loss_occ = _masked_batch_mean(occ_ps, include) if include is not None else _wmean(occ_ps, wo)
    else:
        loss_hm = hm_ps.mean()
        loss_occ = _masked_batch_mean(occ_ps, include) if include is not None else occ_ps.mean()
    loss_imp = _masked_batch_mean(imp_ps, include) if include is not None else imp_ps.mean()
    ...
    if c.occ_k_consistency_weight > 0 and K is not None:
        if tgt_occ.dim() == 3:
            p_occ = torch.sigmoid(recon_occ).sum(dim=-1).clamp(0, 1)
            t_pres = tgt_occ.sum(dim=-1).clamp(0, 1)
            occ_k_each = (p_occ - t_pres).abs().mean(dim=1)
        else:
            t = tgt_occ.clamp(0, 1)
            occ_k_each = (F.relu(0.5 - recon_occ) * t + F.relu(recon_occ + 0.5) * (1 - t)).mean(dim=1)
        occ_k = _masked_batch_mean(occ_k_each, include) if include is not None else occ_k_each.mean()
    ...
    if physics is not None:
        ...
        if tgt_occ.dim() == 3:
            ...
            recon_occ_phys = torch.sigmoid(recon_occ).sum(dim=-1).clamp(0, 1)
            tgt_occ_phys = presence_from_onehot(tgt_occ)
        else:
            recon_occ_phys, tgt_occ_phys = recon_occ, tgt_occ
        ri, cs, ar = physics(recon_hm, recon_occ_phys, recon_imp,
                             target_occ=tgt_occ_phys, target_hm=tgt_hm, pi_freq=pi_freq)
...
def _prepare_batch(batch: dict, c: Config) -> tuple:
    ...
    from experiments.exp060_multitype_occ.codes.occupancy_types import ensure_batch_onehot
    occ = ensure_batch_onehot(occ, int(getattr(c, "n_decap_types", 1)))
...
def _cross_modal(
    model, hm_enc, occ, imp, hm, K, pi, c: Config, imp_std: float, ps: float,
    *, heatmap_only_mask: torch.Tensor | None = None,
) -> torch.Tensor:
    base = unwrap_model(model)
    include = None
    if heatmap_only_mask is not None:
        include = (1.0 - heatmap_only_mask.to(hm.dtype).view(-1)).clamp(0.0, 1.0)
        if float(include.sum()) <= 0:
            return hm.new_zeros(())
    wh, wo = _k_weights(K, c) if c.use_k_weighting else (None, None)
    total = hm.new_zeros(())
    for src in ("heatmap", "impedance"):
        z = base.encode_cross_modal(src, K, pi, heatmap=hm_enc, impedance=imp)
        rh, ro, ri = base.decode(z, K, pi)
        lh = heatmap_loss(rh, hm, c, ps, lite=True)
        if occ.dim() == 3:
            bce = F.binary_cross_entropy_with_logits(ro, occ.clamp(0, 1), reduction="none")
            gamma = c.occupancy_focal_gamma
            if gamma > 0:
                with torch.no_grad():
                    pt = torch.exp(-bce)
                bce = (1.0 - pt).pow(gamma) * bce
        ...
        if include is None:
            ...
            total = total + c.occupancy_weight * _masked_batch_mean(lo, include)
            total = total + c.impedance_weight * _masked_batch_mean(li, include)
    return total / 2.0
...
def _apply_yaml_config(c: Config) -> None:
    """Merge experiment config.yaml (and optional ``VAE_CONFIG_PATH`` overrides)."""
    paths: list[Path] = []
    base = Path(c.experiment_dir) / "config.yaml"
    if base.is_file():
        paths.append(base)
    override = os.environ.get("VAE_CONFIG_PATH")
    if override:
        op = Path(override)
        if op.is_file() and op not in paths:
            paths.append(op)
    if not paths:
        return
    explicit_epochs: set[str] = set()
    for path in paths:
        explicit_epochs |= _merge_yaml_into_config(c, path)
    ...
    *, train: bool, optimizer=None, scaler=None,
    ...
    with ctx:
        for i, batch in enumerate(loader):
            ...
            hm_only = batch.get("heatmap_only")
            if hm_only is not None:
                hm_only = hm_only.to(c.device, non_blocking=c.is_cuda())
            if train:
...
def train_vae() -> None:
    ...
    al_overlay_dir = getattr(c, "al_overlay_data_dir", None)
    if os.environ.get("VAE_SKIP_AL_OVERLAY", "").lower() in ("1", "true", "yes"):
        al_overlay_dir = None
        if is_main_process():
            print("  AL overlay disabled (VAE_SKIP_AL_OVERLAY)", flush=True)
    elif al_overlay_dir:
        overlay_path = Path(al_overlay_dir)
        if not overlay_path.is_dir() or not manifest_path(overlay_path).is_file():
            if is_main_process():
                print(f"  AL overlay missing at {overlay_path} — training without overlay", flush=True)
            al_overlay_dir = None
    train_ld, val_ld = create_multifreq_data_loaders(
        ...
        al_overlay_data_dir=al_overlay_dir,
        al_overlay_sample_weight=float(getattr(c, "al_overlay_sample_weight", 25.0)),
        n_decap_types=int(getattr(c, "n_decap_types", 1)),
    )
    if is_main_process():
        print(
    ...
    for epoch in range(start, c.num_epochs):
        ...
        # Sync after train so rank0-only val cannot race the other rank into broadcast.
        barrier()
        ep = epoch + 1
        ...
        if run_val:
            if is_main_process():
                ...
                # Unwrap DDP: main-only forward must not touch process-group collectives.
                val = _run_epoch(
                    unwrap_model(model), val_ld, c, epoch, beta, 0.0, physics, pw, imp_log_std,
                    hm_log_mean, hm_log_std,
        ...
        if c.eval_off_anchor_mhz and run_val and is_main_process():
            from experiments.exp060_multitype_occ.codes.eval_off_anchor import (
        ...
        # Keep ranks aligned if only rank0 decides to stop.
        early_stopped = bool(broadcast_float(1.0 if early_stopped else 0.0, device=c.device) > 0.5)
        if early_stopped:
            if is_main_process():
    ...
    if is_main_process():
        ...
        if getattr(c, "al_finetune_early_stop", False) and best_oa_path.is_file():
            ...
            print(
                "  Skipping last_model.pt update — no validation metrics "
                f"(trained {max(0, c.num_epochs - start)} epoch(s); start={start}, num_epochs={c.num_epochs})",
                flush=True,
            )
    if is_main_process():
