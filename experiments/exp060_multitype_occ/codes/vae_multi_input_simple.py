...
from experiments.exp060_multitype_occ.codes.graph_occ import OccGraphDecoder, OccGraphEncoder
from experiments.exp060_multitype_occ.codes.graph_imp import ImpSpectrumGraphDecoder, ImpSpectrumGraphEncoder
...
class MultiInputVAE(nn.Module):
    ...
    - Structured latent: z = [z_shared | z_peak | z_spatial].
    - Occupancy expert -> z_shared; impedance expert -> z_shared+z_peak;
      heatmap/freq experts -> z_spatial (and full z for heatmap).
    - Decoder wiring:
    ...
    def __init__(
        ...
        *,
        use_multiscale_film: bool = False,
        ...
        self.imp_peak_dim = int(imp_peak_dim)
        if self.imp_peak_dim < 0:
            raise ValueError(f"imp_peak_dim must be >= 0, got {self.imp_peak_dim}")
        if heatmap_private_dim + self.imp_peak_dim >= latent_dim:
            raise ValueError(
                f"heatmap_private_dim + imp_peak_dim must be < latent_dim; "
                f"got {heatmap_private_dim}+{self.imp_peak_dim} vs {latent_dim}"
            )

        self.latent_dim          = latent_dim
        self.heatmap_private_dim = heatmap_private_dim
        self.shared_latent_dim   = latent_dim - heatmap_private_dim - self.imp_peak_dim
        self.layout_latent_dim   = self.shared_latent_dim + self.imp_peak_dim
        self.modality_dropout = modality_dropout
        self.modality_dropout_protect_heatmap = False
        self.cond_dim         = cond_dim
        self.use_heatmap_film = use_heatmap_film
        self.use_multiscale_film = bool(use_multiscale_film)
        self.use_occ_spatial_decoder = use_occ_spatial_decoder
        self.binary_occupancy_decode = True
        self.occupancy_binary_ste = False
        self.n_occ_classes = int(n_occ_classes)
        if self.n_occ_classes < 1:
            raise ValueError(f"n_occ_classes must be >= 1 (num types), got {self.n_occ_classes}")
        ...
        self.occupancy_encoder = OccGraphEncoder(
            out_dim=64,
            hidden_dim=graph_hidden_dim,
            num_layers=graph_num_layers,
            dropout=graph_dropout,
            n_occ_classes=self.n_occ_classes,
        )

        self.impedance_encoder = ImpSpectrumGraphEncoder(
            out_dim=64,
            hidden_dim=graph_hidden_dim,
            num_layers=graph_num_layers,
            dropout=graph_dropout,
        )
        self.layout_joint_fusion = nn.Sequential(
            nn.Linear(128, 64),
            nn.LayerNorm(64),
            nn.LeakyReLU(0.1),
        )
        ...
        self.impedance_mu     = nn.Linear(enc_in_k, self.layout_latent_dim)
        self.impedance_logvar = nn.Linear(enc_in_k, self.layout_latent_dim)
        ...
        dec_in_imp     = self.layout_latent_dim + cond_dim + 64   # z_layout + K + occ graph ctx
        ...
        # No U-Net skips: decoder is latent+FiLM only (matches stage-2 / generation).
        self.heatmap_dec_refine8 = nn.Sequential(
            nn.Conv2d(128, 128, kernel_size=3, padding=1),
            nn.GroupNorm(8, 128),
            nn.LeakyReLU(),
        )
        self.heatmap_dec_up1 = UpsampleConv2d(128, 64, scale=2, dropout=HM_DROPOUT)
        self.heatmap_dec_refine16 = nn.Sequential(
            nn.Conv2d(64, 64, kernel_size=3, padding=1),
            nn.GroupNorm(8, 64),
            nn.LeakyReLU(),
        )
        self.heatmap_dec_up2 = UpsampleConv2d(64, 32, scale=2, dropout=HM_DROPOUT)
        self.heatmap_dec_refine32 = nn.Sequential(
        ...
        # Multi-scale frequency FiLM: modulate spatial features at every decoder scale so
        # mid-band frequencies can carve distinct structure (not just one 32-ch injection).
        if use_heatmap_film and self.use_multiscale_film:
            self.heatmap_dec_film_bottleneck = FiLM2d(128, 2 * cond_dim)  # 8x8
            self.heatmap_dec_film16 = FiLM2d(64, 2 * cond_dim)            # 16x16
            self.heatmap_dec_film64 = FiLM2d(16, 2 * cond_dim)            # 64x64
        else:
            self.heatmap_dec_film_bottleneck = None
            self.heatmap_dec_film16 = None
            self.heatmap_dec_film64 = None
        ...
        self.occupancy_decoder = OccGraphDecoder(
            dec_in=dec_in_occ,
            hidden_dim=graph_hidden_dim,
            num_layers=graph_num_layers,
            dropout=graph_dropout,
        ...
        self.impedance_decoder = ImpSpectrumGraphDecoder(
            dec_in=dec_in_imp,
            hidden_dim=graph_hidden_dim,
            num_layers=graph_num_layers,
            dropout=graph_dropout,
    ...
    def _zeros_pad(self, batch: int, n: int, *, like: torch.Tensor) -> torch.Tensor:
        if n <= 0:
            return like.new_zeros(batch, 0)
        return torch.zeros(batch, n, device=like.device, dtype=like.dtype)

    def _pad_occ_expert_to_full(self, mu_s: torch.Tensor, logvar_s: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        """Occ expert -> z_shared only; peak + spatial prior."""
        b = mu_s.shape[0]
        z_peak = self._zeros_pad(b, self.imp_peak_dim, like=mu_s)
        z_sp = self._zeros_pad(b, self.heatmap_private_dim, like=mu_s)
        lv_peak = self._zeros_pad(b, self.imp_peak_dim, like=logvar_s)
        lv_sp = self._zeros_pad(b, self.heatmap_private_dim, like=logvar_s)
        return torch.cat([mu_s, z_peak, z_sp], dim=1), torch.cat([logvar_s, lv_peak, lv_sp], dim=1)

    def _pad_imp_expert_to_full(self, mu_l: torch.Tensor, logvar_l: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        """Imp expert -> z_shared + z_peak; spatial prior."""
        b = mu_l.shape[0]
        z_sp = self._zeros_pad(b, self.heatmap_private_dim, like=mu_l)
        lv_sp = self._zeros_pad(b, self.heatmap_private_dim, like=logvar_l)
        return torch.cat([mu_l, z_sp], dim=1), torch.cat([logvar_l, lv_sp], dim=1)

    def _pad_shared_to_full(self, mu_shared: torch.Tensor, logvar_shared: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        """Backward-compatible alias for occ expert padding."""
        return self._pad_occ_expert_to_full(mu_shared, logvar_shared)

    def _layout_latent_slice(self, z: torch.Tensor) -> torch.Tensor:
        return z[:, : self.layout_latent_dim]

    def _encode_layout_joint(
        self, occupancy: torch.Tensor, impedance: torch.Tensor,
    ) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        occ_feat = self.occupancy_encoder(occupancy)
        imp_feat = self.impedance_encoder(impedance)
        joint = self.layout_joint_fusion(torch.cat([occ_feat, imp_feat], dim=-1))
        return occ_feat, imp_feat, joint
    ...
    def _occupancy_to_grid7x8(self, occupancy: torch.Tensor) -> torch.Tensor:
        """Scatter presence → (B, 1, 7, 8) layout grid.

        Accepts (B, 52) presence or (B, 52, C) one-hot (uses 1 - empty channel).
        """
        from experiments.exp060_multitype_occ.codes.occupancy_types import presence_from_onehot
        if occupancy.dim() == 3:
            presence = presence_from_onehot(occupancy)
        else:
            presence = occupancy
        b = presence.shape[0]
        grid = torch.zeros(b, 1, 7, 8, device=presence.device, dtype=presence.dtype)
        hw = self._occ_label_hw
        for i in range(hw.shape[0]):
            h, w = int(hw[i, 0]), int(hw[i, 1])
            grid[:, 0, h, w] = presence[:, i]
        return grid
    ...
    def _encode_heatmap_feat(
        ...
        return self.heatmap_enc2_head(h8)
    ...
    def _decode_heatmap_tensor(
        ...
        hm_feat = self.heatmap_dec_refine8(hm_feat)
        if self.heatmap_dec_film_bottleneck is not None:
            hm_feat = self.heatmap_dec_film_bottleneck(hm_feat, cond)
        hm_feat = self.heatmap_dec_up1(hm_feat)
        hm_feat = self.heatmap_dec_refine16(hm_feat)
        ...
        if self.heatmap_dec_film16 is not None:
            hm_feat = self.heatmap_dec_film16(hm_feat, cond)

        hm_feat = self.heatmap_dec_up2(hm_feat)
        hm_feat = self.heatmap_dec_refine32(hm_feat)
        ...
        if self.heatmap_dec_film64 is not None:
            hm_feat = self.heatmap_dec_film64(hm_feat, cond)
    ...
    def encode(self, heatmap, occupancy, impedance, K, PI_freq):
        ...
        Args:
            heatmap:   (B, 1, 64, 64)
            occupancy: (B, 52, T) one-hot (empty=zeros); legacy (B, 52) also accepted
            impedance: (B, 1, 231)
            K:         (B,) LongTensor
        ...
        occ_feat, imp_feat, joint_feat = self._encode_layout_joint(occupancy, impedance)

        k_emb  = self.k_embedding(K)
        hm_c   = torch.cat([heatmap_feat, cond_hm], dim=1)
        occ_c  = torch.cat([joint_feat, k_emb], dim=1)
        imp_c  = torch.cat([joint_feat, k_emb], dim=1)
        ...
        imp_mu, imp_lv = self._pad_imp_expert_to_full(imp_mu_s, imp_lv_s)
    ...
    def decode(
        ...
        z_layout = self._layout_latent_slice(z)
        dec_occ  = torch.cat([z_shared, k_emb], dim=1)

        occ_for_encoder = occupancy
        occ_for_hm = occupancy
        if occupancy is not None and getattr(self, "binary_occupancy_decode", False):
            from experiments.exp060_multitype_occ.codes.occupancy_types import (
                hard_onehot_from_probs,
                occupancy_for_heatmap_decode,
                presence_from_onehot,
            )
            ste = getattr(self, "occupancy_binary_ste", False)
            if occupancy.dim() == 3:
                hard = hard_onehot_from_probs(occupancy, K)
                if ste and occupancy.requires_grad:
                    hard = hard + (occupancy - occupancy.detach())
                occ_for_encoder = hard
                occ_for_hm = presence_from_onehot(hard)
            else:
                occ_for_hm = occupancy_for_heatmap_decode(
                    occupancy, K, force_hard=True, ste=ste,
                )
                occ_for_encoder = occ_for_hm

        hm_recon = self._decode_heatmap_tensor(z, cond, occupancy=occ_for_hm)

        occ_recon = self.occupancy_decoder(dec_occ)  # (B, 52, C) logits
        if occ_for_encoder is not None:
            occ_for_imp = occ_for_encoder
        else:
            from experiments.exp060_multitype_occ.codes.occupancy_types import logits_to_probs
            occ_for_imp = logits_to_probs(occ_recon)
        occ_ctx = self.occupancy_encoder(occ_for_imp)
        dec_imp = torch.cat([z_layout, k_emb, occ_ctx], dim=1)
    ...
    def decode_impedance(self, z: torch.Tensor, K, PI_freq=None) -> torch.Tensor:
        ...
        z_layout  = self._layout_latent_slice(z)
        occ_ctx   = torch.zeros(B, 64, device=z.device, dtype=z.dtype)
        dec_imp   = torch.cat([z_layout, k_emb, occ_ctx], dim=1)
        imp_recon = self.impedance_decoder(dec_imp).unsqueeze(1)                     # (B, 1, 231)
    ...
    def _encode_impedance_expert(self, impedance, K: torch.Tensor, *, return_feat: bool = False):
        ...
        imp_feat = self.impedance_encoder(imp_in)
        k_emb  = self.k_embedding(K)
        # impedance-only path: use imp_feat as both branches for joint
        joint = self.layout_joint_fusion(torch.cat([imp_feat, imp_feat], dim=-1))
        feat_c = torch.cat([joint, k_emb], dim=1)
        mu_s   = self.impedance_mu(feat_c)
        lv_s   = self.impedance_logvar(feat_c).clamp(-6.0, 1.0)
        mu_full, lv_full = self._pad_imp_expert_to_full(mu_s, lv_s)
        if return_feat:
            return mu_full, lv_full, imp_feat
    ...
    def encode_cross_modal(self, source: str, K, PI_freq,
        ...
        if source == 'heatmap':
            ...
            k_emb    = self.k_embedding(K)
            _, _, joint_feat = self._encode_layout_joint(occupancy, impedance)
            occ_c    = torch.cat([joint_feat, k_emb], dim=1)
            occ_mu_s = self.occupancy_mu(occ_c)
    ...
    def forward(self, heatmap, occupancy, impedance, K, PI_freq):
        ...
        hm_recon, occ_recon, imp_recon = self.decode(
            z, K, PI_freq, occupancy=occupancy,
        )
    ...
    def _heatmap_dropout_module_names(self) -> tuple[str, ...]:
        ...
        return (
            ...
            "heatmap_dec_up2",
            "heatmap_dec_up3",
            "heatmap_dec_refine16",
            "heatmap_dec_refine32",
        ...
        with torch.no_grad():
            ...
            if mode == "encode":
                ...
                if occupancy is not None and impedance is None:
                    ...
                    from experiments.exp060_multitype_occ.codes.occupancy_types import logits_to_probs
                    occupancy = logits_to_probs(occ_logits)
                    impedance = imp0
            ...
            # Multi-type: return class probabilities (B,52,C); legacy binary kept via C=2.
            from experiments.exp060_multitype_occ.codes.occupancy_types import logits_to_probs
            occupancy_out = logits_to_probs(occupancy_out)
