...
from experiments.exp060_multitype_occ.codes.vae_multi_input_simple import MultiInputVAE

_EXP_LV_MIN, _EXP_LV_MAX = -6.0, 1.0


class MultiInputVAEPoeFreq(MultiInputVAE):
    """PoE fusion with PI_freq expert on private dims (no U-Net skips)."""
    ...
    def __init__(
        ...
        *,
        ...
        super().__init__(
            ...
            imp_peak_dim=imp_peak_dim,
            n_occ_classes=n_occ_classes,
        )
    ...
    def _pad_freq_private_to_full(
        ...
        z_layout = torch.zeros(b, self.layout_latent_dim, device=dev, dtype=dt)
        lv_layout = torch.zeros(b, self.layout_latent_dim, device=dev, dtype=dt)
        mu_full = torch.cat([z_layout, mu_private], dim=1)
        lv_full = torch.cat([lv_layout, logvar_private], dim=1)
    ...
    def encode_layout_latent_full(
        ...
        *,
        ...
        occ_feat, imp_feat, joint_feat = self._encode_layout_joint(occupancy, impedance)
        occ_c = torch.cat([joint_feat, k_emb], dim=1)
        occ_mu, occ_lv = self._pad_occ_expert_to_full(
            self.occupancy_mu(occ_c),
            self.occupancy_logvar(occ_c).clamp(_EXP_LV_MIN, _EXP_LV_MAX),
        )
        imp_c = torch.cat([joint_feat, k_emb], dim=1)
        imp_mu, imp_lv = self._pad_imp_expert_to_full(
            self.impedance_mu(imp_c),
            self.impedance_logvar(imp_c).clamp(_EXP_LV_MIN, _EXP_LV_MAX),
        )
    ...
    def decode_heatmap(
        ...
        *,
        ...
        cond = self._cond_emb(K, PI_freq)
        return self._decode_heatmap_tensor(z, cond, occupancy=occupancy)
