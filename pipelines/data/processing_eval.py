"""Build held-out evaluation dataset excluding training occupancy combinations.

Run:
    python pipelines/data/processing_eval.py"""
...
from libs.data_creation.impedance import EXPECTED_IMP_LENGTH, read_impedance_file
...
# =============================================================================
# CONFIGURATION — edit these before running: python pipelines/data/processing_eval.py
# =============================================================================

# Raw dataset root (prefer DATA_ROOT env var)
RAW_ROOT = next(
    (
        Path(p)
        for p in [
    ...
    *,
    ...
    if not tasks:
        ...
        write_dataset_meta(
            out_root,
            stage="raw",
            source_script="pipelines/data/processing_eval.py",
            extra={"evaluation_holdout": True},
        )
        return 0
