"""Build multifreq training dataset (layout-centric storage).
...
from libs.data_creation.heatmap import create_Heatmaps, load_mask_board
from libs.data_creation.impedance import read_impedance_file, EXPECTED_IMP_LENGTH
from libs.data_creation.csv_to_occupancy import create_occupancy_vector
...
from src_vae.others.multifreq_layout_store import (  # noqa: E402
    manifest_design_ids,
    manifest_layout_keys,
    manifest_path,
    manifest_pi_to_design_id,
    resolve_manifest_design_id,
)
...
FRAME_PATH = repo_path("configs") / "binary_mask.npy"
TRAIN_DATA_ROOT = repo_path("datasets") / "data_multifreq_train"
...
# =============================================================================
# CONFIGURATION — edit these before running: python pipelines/data/processing_multifreq.py
# =============================================================================

# Output dataset root (layout store: layouts/, heatmap/, PI_freq/, manifest.csv)
OUTPUT_ROOT = TRAIN_DATA_ROOT

# Raw ECADStar export root (None = auto-detect from DATA_ROOT env or Windows path)
DATA_ROOT: str | Path | None = None

# Append new rows to existing output instead of wiping (use with new MHz anchors)
APPEND = True

# Only process these MHz when appending (None = all anchors from multifreq_anchors.yaml)
ONLY_FREQS_MHZ: list[float] | None = [350.0, 370.0, 390.0, 420.0, 450.0]

# When APPEND=True: only add rows for design_id already in output manifest (subset-safe)
RESTRICT_APPEND_TO_MANIFEST = True

# Cap total manifest rows (None = no limit)
MAX_SAMPLES: int | None = None

# Parallel workers (env NUM_WORKERS overrides if set)
...
def create_training_dataset(
    ...
    *,
    ...
    restrict_append_to_manifest: bool = RESTRICT_APPEND_TO_MANIFEST,
) -> int:
    ...
    if append and restrict_append_to_manifest and manifest_path(root).is_file():
        by_pi_decap = manifest_layout_keys(root)
        by_pi = manifest_pi_to_design_id(root)
        if by_pi_decap or by_pi:
            before = len(all_samples)
            matched: list[dict] = []
            for s in all_samples:
                did = resolve_manifest_design_id(s, by_pi_decap, by_pi)
                if did is None:
                    continue
                s = dict(s)
                s["design_id"] = did
            ...
            all_samples = matched
            print(
                f"  Restricted to manifest layouts (pi/decap match): "
                f"{before:,} → {len(all_samples):,} rows, "
                f"{len({s['design_id'] for s in all_samples}):,} unique design_id "
                f"(manifest has {len(manifest_design_ids(root)):,})"
            )
    ...
    write_dataset_meta(
        ...
        extra={"anchor_mhz_config": [FREQ_HZ[f] / 1e6 for f in FREQ_LABELS]},
    )
...
if __name__ == "__main__":
    if ONLY_FREQS_MHZ:
        _refresh_freq_tables(only_mhz=ONLY_FREQS_MHZ)
    ...
    print(f"Data root  : {DATA_ROOT or DEFAULT_DATA_ROOT or 'Not found'}")
    print(f"Mode       : {'append' if APPEND else 'rebuild'}")
    if APPEND:
        print(f"Append restrict: {RESTRICT_APPEND_TO_MANIFEST} (existing manifest design_id only)")
    ...
    count = create_training_dataset(
        ...
        verbose=True,
        overwrite_output=False if APPEND else OVERWRITE_OUTPUT,
        append=APPEND,
        only_freq_mhz=ONLY_FREQS_MHZ,
        restrict_append_to_manifest=RESTRICT_APPEND_TO_MANIFEST,
    )

    print(f"\nProcessed {count} samples")
    print(f"Next: python pipelines/normalize/multifreq.py{'  # set APPEND=True there too' if APPEND else ''}")
