"""One-shot installer for append_merged_combinations_multifreq.py (pipelines/data is cursorignored)."""
from pathlib import Path

DEST = Path(__file__).resolve().parents[1] / "data" / "append_merged_combinations_multifreq.py"
...
MHZ_LIST: list[float] | None = [170.0]

APPEND_TAG: str | None = "merged_170"
DRY_RUN = False
SKIP_EXISTING = True
...
def _validate_raw_slice(
    ...
    *,
    ...
    for e in index_entries:
        pi = int(e["pi_number"])
        if combinations_use_local_pi and e["segment"] == "combinations":
            pi = int(e["local_row"]) + 1
        needed_pi.add(pi)
...
def main() -> None:
    ...
    DEST.chmod(0o755)
    print(f"Installed {DEST} ({len(CONTENT.splitlines())} lines)")
