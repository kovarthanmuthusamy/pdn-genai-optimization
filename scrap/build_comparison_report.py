"""Build a self-contained HTML report of generated-vs-real comparisons.

Workflows:
- ``run_all_k`` (default when run as script): heatmap + impedance + occupancy tabs
- ``multifreq_heatmap_sweep``: fixed K, PI-freq sweep, heatmap tab only

Run:
    python scrap/build_comparison_report.py
    python scrap/build_comparison_report.py --workflow multifreq_heatmap_sweep
"""
...
def _write_markdown_summary(
    *,
    ...
    for freq_label, freq_sub in freq_scan:
        ...
        for k in k_range:
            rel = (
                f"{freq_sub}/K{k}/{HEATMAP_IMG}"
                if freq_sub
                else f"K{k}/{HEATMAP_IMG}"
            )
            img_path = freq_dir / f"K{k}" / HEATMAP_IMG
...
def build_multifreq_sweep_report(
    *,
    ...
    """HTML + markdown index for exp038 multifreq heatmap sweep (fixed K)."""
    ...
    return render_comparison_report(
        ...
        k_max=K_VALUE,
        tabs=tabs,
        title="PI-Distribution sweep — Generated vs Real",
        subtitle=f"Fixed K = {K_VALUE} · {len(freq_scan)} PI frequencies",
        report_copy_dest=dest,
    )


def build_report(workflow: str = "run_all_k") -> Path:
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Build comparison HTML report")
    ap.add_argument(
        "--workflow",
        choices=("run_all_k", "multifreq_heatmap_sweep"),
        default="run_all_k",
    )
    args = ap.parse_args()
    build_report(args.workflow)
