"""Move PI-* outputs from an ECADStar .emc directory into the local Real/ folder(s).

Goal
- "Cut" (move) items named like PI-1, PI-2, ... from a source `ProKI_design.emc` directory
    and place them into generated `K*/Real/` folders (or `freq_*MHz/K*/Real/`).

Workflows (set ``WORKFLOW`` below)
- ``run_all_k`` — K sweep / multi-freq from ``run_all_k.py`` (heatmap + impedance: 2 PI per sample)
- ``multifreq_heatmap_sweep`` — fixed K, PI-freq sweep from ``run_multifreq_heatmap_sweep.py``
    (PI-Distribution only: 1 PI per sample → ``Heatmap_real_*`` only)

Two modes (run_all_k)
- Single-K mode: moves PI-1..PI-(2*N) into `K{K_VALUE}/Real/` and renames them.
- Combined-PEB mode: if you ran one `.peb` that contains *all* K (K=1..52) in order,
    this script can split PI outputs across `K1/..K52/Real/` and rename them per folder.
Notes
- The ECADStar `.emc` path you provided is Windows-style. If you're running this script on WSL,
  it will also try the `/mnt/<drive>/...` equivalent automatically.
- By default this script only looks at the *top-level* items inside the `.emc` directory.

Run
    python scrap/move_pi_to_real.py

Or call from Python
    from scrap.move_pi_to_real import move_pi_outputs_for_k, move_pi_outputs_for_k_range
    move_pi_outputs_for_k(5)
    move_pi_outputs_for_k_range(1, 52)
"""
