#!/usr/bin/env python3
"""Normalize a processed dataset using existing training normalization stats.

Why
---
Your VAE model expects *normalized* inputs:
- heatmap: (B, 1, 64, 64)
- impedance: (B, 3, 231)
- occupancy: (B, 52)

But the held-out dataset created by `Data_Creation/Data_processing_eval.py` is in
*raw processed* form:
- heatmap: (2, 64, 64)  [raw channel + mask]
- impedance: (231, 1) or (231,)
- occupancy: (52,)

This script converts any dataset root with the above raw structure into a
normalized dataset root, using the *training* normalization statistics
(e.g. `datasets/data_norm/normalization_stats.json`).

Normalization
-------------
- Heatmap: log(1+x) z-score on foreground pixels; background set to sentinel.
- Impedance: log z-score; outputs 3 channels (z, d1, d2).
- Occupancy: copied as float32.

Run
---
  python scripts/normalize_dataset_with_stats.py \
    --in-root datasets/data_eval \
    --out-root datasets/data_eval_norm \
    --stats-json datasets/data_norm/normalization_stats.json

This script is resume-friendly by default.
"""
...
def main() -> None:
    ...
    ap.add_argument(
        "--stats-json",
        type=Path,
        default=Path("datasets/data_norm/normalization_stats.json"),
        help="Training normalization stats to reuse (avoid test leakage).",
    )
    ap.add_argument("--overwrite", action="store_true", help="Delete out-root before writing")
    ap.add_argument("--max-samples", type=int, default=None, help="Optional cap for debugging")
    ...
    hm_stats, imp_stats = _load_stats(args.stats_json)
    ...
    if args.max_samples is not None:
        ids = ids[: max(0, int(args.max_samples))]
    ...
    for i, sid in enumerate(ids):
        ...
        if args.print_every and (i + 1) % int(args.print_every) == 0:
