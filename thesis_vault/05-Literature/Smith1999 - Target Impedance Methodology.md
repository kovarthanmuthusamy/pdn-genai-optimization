---
title: Smith1999 - Target Impedance Methodology
type: literature
tier: core
year: 1999
tags: [literature, core, pdn-domain]
---

# Power distribution system design methodology and capacitor selection for modern CMOS technology

**Authors:** L. D. Smith, R. E. Anderson, D. W. Forehand, T. J. Pelc, T. Roy  
**Year:** 1999 · **Venue:** IEEE Trans. Advanced Packaging 22(3)  
**Link:** https://doi.org/10.1109/6040.784471  
**Tier:** CORE — belongs in the literature review

## Why it matters here

The origin of the target-impedance design rule Z(f) <= Z_target(f) that your feasibility constraint encodes verbatim. Cite this when you introduce the mask in Chapter 1 — it establishes that your objective is the field's standard acceptance criterion, not one you invented.

## Where it goes in the thesis

Ch.1 problem statement, when defining Z_target and the 231-bin mask.

## Links

- [[framework-overview]]
- [[latent-optimization]]
- [[Literature Review]]

---

⚠️ Title/venue from search results, not fetched in full — **verify author list, year and page numbers before citing**.
