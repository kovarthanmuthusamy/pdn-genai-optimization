...
type: concept
source: docs/experiment-lineage.md
tags: [concept, thesis]
---

> [!info] Mirror of `docs/experiment-lineage.md` — edit the source file, then re-run `tools/build_vault.py`.
...
Detailed architecture for exp054–057: [[model-architecture|model-architecture.md]].
...
| **exp056_graph_vae** | PCB-slot occupancy GNN | Spatial inductive bias on slots; see [[gnn-rationale|gnn-rationale.md]] |
...
2. Use **exp048–050** when discussing normalization and Tier-A losses ([[normalization-and-losses|normalization-and-losses.md]]).
3. Use **exp058** only as a legacy AL-path ablation (occ-only comparable to 059).
4. Always name the **checkpoint folder** and data split when reporting numbers. See [[gp-error-surrogate|gp-error-surrogate.md]].
