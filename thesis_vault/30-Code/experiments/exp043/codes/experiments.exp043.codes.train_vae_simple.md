...
type: code
path: experiments/exp043/codes/train_vae_simple.py
...
**Source:** `experiments/exp043/codes/train_vae_simple.py` · 1708 lines
**Experiment:** [[exp043]]
...
- **`_iter_expert_stats(expert_stats: dict | None)`** — Yield ``(name, mu, logvar)`` — skips ``heatmap_skips`` and other metadata.
- **`_expert_mu_lv_pairs(expert_stats: dict | None)`** — (mu, logvar) tuples only — skips ``heatmap_skips`` and other metadata.
...
- **`_amp_dtype(c: Config)`**
- **`_layout_pi_ref_norm(pi: torch.Tensor, c: Config)`** — Sweep-aligned π_ref for layout encode (default 200 MHz from config).
- **`_teacher_skips(expert_stats: dict, c: Config)`**
- **`_cross_freq_heatmap_loss(model, z: torch.Tensor, K: torch.Tensor, pi_alt: torch.Tensor, hm_alt: torch.Tensor, c: Config, ps: float, *, dynrange_weight: float | None=None, occupancy: torch.Tensor | None=None, heatmap_skips: dict[str, torch.Tensor] | None=None)`** — Decode same z at alternate PI_freq; target is ground-truth heatmap at that MHz.
- **`_layout_train_prob(epoch: int, c: Config)`** — Early training favors posterior z (heatmap encoder recon); later matches layout sweep.
- **`_forward_train_batch(model, hm_enc: torch.Tensor, occ: torch.Tensor, imp: torch.Tensor, K: torch.Tensor, pi: torch.Tensor, c: Config, *, train: bool, epoch: int=0)`** — Encode for KL; decode from layout or posterior z with optional PI_freq jitter.
- **`_prepare_batch(batch: dict, c: Config)`**
- **`_cross_modal(model, hm_enc, occ, imp, hm, K, pi, c: Config, imp_std: float, ps: float)`**
- **`_clean_previous_run(exp: Path)`** — Deprecated: exp043 no longer deletes old runs (kept for backward compatibility).
- **`_new_run_dir(exp: Path)`** — Create a unique run directory under exp/runs/ (never deletes existing data).
...
- **`_on_stats_loaded(c: Config, raw: dict)`** — Apply global-max heatmap patches when dataset uses gmax normalization.
- **`build_vae_model(c: Config)`** — Build exp043 PoE-freq model with optional heatmap-private latent tail.
- **`train_vae()`**
...
- [[experiments.exp043.codes.impedance_spectrum_loss]]
