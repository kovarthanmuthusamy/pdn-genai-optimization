...
type: code
path: experiments/exp060_multitype_occ/codes/dataloader_base.py
...
**Source:** `experiments/exp060_multitype_occ/codes/dataloader_base.py` · 427 lines
...
- **`set_n_decap_types(n: int)`**
- **`_occ_to_onehot_tensor(occ: torch.Tensor)`**
...
- **`_split_indices_by_design(design_keys: list[str], train_split: float, seed: int)`** — True layout holdout: each design_id (all its MHz rows) goes entirely to
